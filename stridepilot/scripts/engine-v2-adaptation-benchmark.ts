import {
  ...
  generateEngineV2Plan,
  referenceRunnerBenchmarks,
  updatePlanAfterFeedback,
} from "../src/lib/engine-v2";
import type { FeedbackSignal, RunnerInput, RunnerStatus } from "../src/lib/engine-v2";
...
type Scenario = {
  ...
  checks: Array<{
    label: string;
    pass: (summary: ScenarioSummary) => boolean;
  }>;
};
...
function buildFeedback(
  plan: ReturnType<typeof generateEngineV2Plan>,
  overrides: Array<Partial<FeedbackSignal>>,
): FeedbackSignal[] {
  ...
  return overrides.map((override, index) => {
    const session = sessions[index] ?? sessions.at(-1) ?? { weekIndex: 1, sessionFamily: "easy_run" as const };
...
function summarizeScenario(
  ...
  const evaluation = evaluateRunnerStatus(feedback, plan, plan.classification);
  const adapted = updatePlanAfterFeedback(plan, feedback);
  const baseIndex = 1;
  return {
...
const baseInput: RunnerInput =
  referenceRunnerBenchmarks.find((fixture) => fixture.id === "recreational_10k_improve")?.input ??
  referenceRunnerBenchmarks[2]!.input;

const plan = generateEngineV2Plan(baseInput);
...
for (const scenario of scenarios) {
  ...
  console.log(
    `deltas: volume=${roundPct(summary.volumeDeltaPct)} | long_run=${roundPct(summary.longRunDeltaPct)} | intensity=${roundPct(summary.intensityDeltaPct)} | sessions=${summary.sessionsDelta >= 0 ? "+" : ""}${summary.sessionsDelta}`,
  );
