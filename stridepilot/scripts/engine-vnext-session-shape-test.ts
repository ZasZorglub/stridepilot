...
import { expandStructure, mergeAdjacentWorkoutSteps } from "../src/lib/coach/mapToAppPlan";
import type { Goal, RunnerProfile, WorkoutSession } from "../src/lib/types";
import { generateFallbackPlan } from "../src/lib/plan";

function countAdjacentIdenticalRunBlocks(session: WorkoutSession): number {
  let count = 0;
...

{
  ...
  assert.deepEqual(
    merged.map((step) => [step.type, step.durationSec]),
    [
      ["warmup", 6 * 60],
      ["run", 28 * 60],
      ["cooldown", 3 * 60],
...

{
  ...
  assert.ok(
    runWalk.filter((step) => step.type === "run").every((step) => step.heartRateGuidance?.zoneLabel === "Zone 2"),
    "beginner and comeback run-walk sessions should keep aerobic zone guidance on the run blocks",
  );
}

{
  ...
  assert.equal(
    merged.filter((step) => step.type === "run").length,
    4,
    "real interval sessions should keep distinct work reps",
  );
  assert.equal(
    merged.filter((step) => step.type === "walk").length,
    3,
    "real interval sessions should keep distinct recovery blocks",
  );
  assert.ok(
    merged.filter((step) => step.type === "run").every((step) => step.heartRateGuidance?.zoneLabel === "Zone 4"),
    "real interval sessions should preserve stronger guidance where the workout semantics are genuinely quality-focused",
  );
}

{
  const merged = mergeAdjacentWorkoutSteps([
  ...
  assert.deepEqual(
    merged.map((step) => step.type),
    ["walk", "run", "walk"],
    "walk/run/walk structures should remain intact when the behavior genuinely changes",
  );
}
...
for (const benchmark of benchmarkProfiles) {
  const plan = generateFallbackPlan(benchmark.runnerProfile, benchmark.goal);

  assert.ok(plan.weeks >= 6, `${benchmark.id} should still produce a meaningful multi-week plan`);
  assert.ok(plan.sessions.length >= plan.weeks * 2, `${benchmark.id} should produce a viable weekly structure`);
  assert.ok(plan.sessions.length <= plan.weeks * 4, `${benchmark.id} should stay within sane session counts`);

  for (const session of plan.sessions) {
    assert.equal(
      countAdjacentIdenticalRunBlocks(session),
      0,
      `${benchmark.id} should not produce nonsensical adjacent identical run blocks in ${session.title}`,
    );
    assert.ok(session.steps.length >= 1, `${benchmark.id} should keep readable session shapes`);
  }
}
