import { adaptPlanFromFeedback, createInitialCapabilityState } from "../src/lib/coach";
import type { WorkoutFeedback as CoachWorkoutFeedback } from "../src/lib/coach/capability";
import { buildGoalPlan } from "../src/lib/coach/build5kPlan";
import { interpretRunnerProfile } from "../src/lib/coach/interpreter";
import { mapCoachPlanToAppPlan } from "../src/lib/coach/mapToAppPlan";
import type { Goal, RunnerProfile as AppRunnerProfile, TrainingPlan } from "../src/lib/types";

type PersonaCategory = "beginner" | "intermediate" | "experienced" | "edge";
...
type Finding = {
  severity: Severity;
  area: "engine" | "explanation" | "product" | "harness";
  summary: string;
  scenarioId: string;
};
...
type AdaptationPattern = {
  name: string;
  personaId: string;
  feedbacks: Array<Omit<CoachWorkoutFeedback, "sessionId"> & { target: "quality" | "long" | "any" }>;
};
...
function deriveCoachGoal(goal: Goal, profile: AppRunnerProfile) {
...
function buildAppPlan(persona: Persona): TrainingPlan {
  ...
  return mapCoachPlanToAppPlan(buildGoalPlan(coachProfile, coachGoal), coachGoal);
}

function sessionType(session: TrainingPlan["sessions"][number]): "interval" | "tempo" | "strides" | "benchmark" | "long" | "recovery" | "run-walk" | "easy" {
  ...
  if (text.includes("strides")) return "strides";
  ...
  if (text.includes("run-walk")) return "run-walk";
  return "easy";
}

function runMinutes(session: TrainingPlan["sessions"][number]): number {
  return Math.round(session.steps.filter((step) => step.type === "run").reduce((sum, step) => sum + step.durationSec, 0) / 60);
}

function weekNumbers(plan: TrainingPlan): number[] {
  return [...new Set(plan.sessions.map((session) => session.week))].sort((a, b) => a - b);
}

function weekSessions(plan: TrainingPlan, week: number): TrainingPlan["sessions"] {
  return plan.sessions.filter((session) => session.week === week);
}

function weekLoad(plan: TrainingPlan, week: number): number {
  return weekSessions(plan, week).reduce((sum, session) => sum + session.loadScore, 0);
}

function longRunMinutesForWeek(plan: TrainingPlan, week: number): number {
  return Math.max(0, ...weekSessions(plan, week).filter((session) => sessionType(session) === "long").map(runMinutes));
}

function formatWeekMix(plan: TrainingPlan, week: number): string {
  const sessions = weekSessions(plan, week);
  if (sessions.length === 0) return "ingen pas";
  return sessions.map((session) => `${sessionType(session)} ${runMinutes(session)}`).join(" · ");
}

function workoutCounts(plan: TrainingPlan): Record<string, number> {
  return plan.sessions.reduce<Record<string, number>>((acc, session) => {
...
function expectedQualityDensity(persona: Persona): { minQualityWeeks: number; note: string } {
...
function evaluatePlanQuality(persona: Persona, plan: TrainingPlan): { summary: string; findings: Finding[] } {
  ...
  const qualityWeeks = weeks.filter((week) =>
    weekSessions(plan, week).some((session) => ["interval", "tempo", "strides"].includes(sessionType(session))),
  ).length;
  const performanceGoal = persona.goal.goalType === "pr" || persona.goal.goalType === "target_time";
  const strongBackground =
    persona.runnerProfile.runningExperience === "ovet" ||
  ...
  const beginnerLike =
    persona.runnerProfile.runningExperience === "nybegynder" ||
  ...
  const tooLongQuality = plan.sessions.some((session) => {
    ...
    if (type === "interval") {
      if (persona.goal.distance === "5K") return minutes > 50;
      if (persona.goal.distance === "10K") return minutes > 60;
      if (persona.goal.distance === "Halvmaraton") return minutes > 65;
      return minutes > 70;
    }
    if (type === "tempo") {
      if (persona.goal.distance === "5K") return minutes > 45;
      if (persona.goal.distance === "10K") return minutes > 55;
      if (persona.goal.distance === "Halvmaraton") return minutes > 65;
      return minutes > 75;
    }
    return false;
...
function evaluateProgression(persona: Persona, plan: TrainingPlan): { summary: string; findings: Finding[]; checkpoints: Array<{ label: string; week: number; load: number; mix: string; longRun: number }> } {
  const findings: Finding[] = [];
  const weeks = weekNumbers(plan);
...
function selectSession(plan: TrainingPlan, target: "quality" | "long" | "any") {
...
function raceSpecificityLooksPreserved(plan: TrainingPlan, persona: Persona): string {
...
function judgeAdaptation(mode: string, beforeMix: string, afterMix: string): string {
  if (mode === "recovery_microcycle") return "Coach-like and clearly caution-oriented.";
  if (mode === "down_shift") return beforeMix !== afterMix ? "Coach-like and structurally lighter." : "A bit too cosmetic.";
  if (mode === "resume_build") return "Coach-like if the rebuild remains controlled.";
  if (mode === "progress") return "Useful if the extra challenge becomes more specific, not just more load.";
  return "Stable and acceptable if feedback was neutral.";
}

function runAdaptationPattern(persona: Persona, pattern: AdaptationPattern) {
  let plan = buildAppPlan(persona);
  let capability = createInitialCapabilityState(plan);
  let mode = "hold";
  let reason = "";
  let lastResult: ReturnType<typeof adaptPlanFromFeedback> | null = null;
  ...
  for (const feedback of pattern.feedbacks) {
    ...
    const result = adaptPlanFromFeedback(plan, { ...feedback, sessionId: session.id }, capability);
...
function simulateDivergence(persona: Persona, feedbacks: DivergencePair["runnerA"]["feedbacks"]) {
  let plan = buildAppPlan(persona);
  let capability = createInitialCapabilityState(plan);
  let lastResult: ReturnType<typeof adaptPlanFromFeedback> | null = null;
...
const beginnerSafetyScenarios: BeginnerSafetyScenario[] = [
  {
    id: "cannot_run_1_min",
...
function personaProfileLine(persona: Persona): string {
  return [
...
function describeWeek(plan: TrainingPlan, week: number): string {
  return formatWeekMix(plan, week);
}

function beginnerSafetyJudgment(plan: TrainingPlan): string {
...
for (const persona of personas) {
  ...
  const coachVerdict =
    quality.findings.some((finding) => finding.severity === "critical")
      ? "Not ready as-is."
