import { buildTimelineRecommendation, classifyRunner, generateEngineV2Plan } from "../src/lib/engine-v2";
...
for (const fixture of fixtures) {
  ...
  const { goalClassification, recommendation } = buildTimelineRecommendation(fixture.input, classification);
  const plan = generateEngineV2Plan(fixture.input);

  console.log(`\n=== ${fixture.name} ===`);
  console.log(
    `classification=${classification.traits.primaryRunnerType} | level=${classification.traits.runnerLevel} | consistency=${classification.traits.consistencyProfile} | risk=${classification.traits.injuryRiskScore}`,
  );
  console.log(
    `goal=${goalClassification.demand} | complexity=${goalClassification.recommendedComplexity} | specificity=${goalClassification.specificityNeed}`,
  );
  console.log(
    `timeline=recommended:${recommendation.recommendedDurationWeeks}w final:${recommendation.finalDurationWeeks}w realism:${recommendation.realism} progression:${recommendation.recommendedProgressionMode}`,
  );
  console.log(
    `sessions=start:${recommendation.startingSessionsPerWeek} rec:${recommendation.recommendedSessionsPerWeek} peak:${recommendation.peakSessionsPerWeek}`,
  );
  ...
  console.log(
    `materialized_plan=goalDate:${plan.resolvedInput.goalDate} weeks:${plan.phasePlan.totalWeeks} planType:${plan.planTypeDecision.planType}`,
  );
}
