...
exports.capabilityStorageKey = capabilityStorageKey;
exports.defaultRunnerTraits = defaultRunnerTraits;
exports.deriveRunnerTraits = deriveRunnerTraits;
exports.createInitialCapabilityState = createInitialCapabilityState;
function capabilityStorageKey(profileId) {
...
function sessionContinuousMinutes(session) {
    let current = 0;
    let longest = 0;
...
function deriveRunnerTraits(history) {
    if (history.length === 0) {
    ...
    const tooEasyCount = history.filter((entry) => entry.completed && entry.completionPct >= 90 && entry.difficulty === "easy" && entry.energy === "high" && entry.pain === "none").length;
    const strongSessionCount = history.filter((entry) => entry.completed && entry.completionPct >= 90 && entry.difficulty !== "very_hard" && entry.energy !== "low" && entry.pain !== "moderate" && entry.pain !== "high").length;
    const qualityHistory = history.filter((entry) => entry.sessionKind === "quality");
    const qualityTolerance = qualityHistory.length === 0
        ? clampTrait(3 + (tooEasyCount - cautionCount) * 0.15)
        : clampTrait(3 +
            average(qualityHistory.map((entry) => {
                let score = 0;
                if (entry.completed && entry.completionPct >= 90)
                    score += 0.45;
                if (entry.difficulty === "easy")
                    score += 0.35;
                if (entry.difficulty === "hard")
                    score -= 0.25;
                if (entry.difficulty === "very_hard")
                    score -= 0.55;
                if (entry.energy === "high")
                    score += 0.2;
                if (entry.energy === "low")
                    score -= 0.35;
                if (entry.pain === "moderate")
                    score -= 0.45;
                if (entry.pain === "high")
                    score -= 0.8;
                return score;
    ...
    const longRunTolerance = longRunHistory.length === 0
        ? 3
        : clampTrait(3 +
            average(longRunHistory.map((entry) => {
                let score = 0;
                if (entry.completed && entry.completionPct >= 90)
                    score += 0.45;
                if (entry.energy === "high")
...
function createInitialCapabilityState(plan) {
