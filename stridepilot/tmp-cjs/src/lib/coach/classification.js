...
exports.classifyRunnerCategory = classifyRunnerCategory;
exports.runnerCategoryReason = runnerCategoryReason;
exports.choosePlanType = choosePlanType;
function continuousCapacityMinutes(profile) {
    ...
    const fromSpecificity = profile.runningSpecificity >= 5
        ? 45
...
function classifyRunnerCategory(profile) {
    ...
    if (continuousMinutes < 10 &&
        runs <= 2 &&
        volume <= 8 &&
        longest <= 12) {
        return "run_walk_beginner";
    }
    if (continuousMinutes < 25 &&
        runs <= 3 &&
        volume <= 16 &&
        longest <= 25) {
...
function runnerCategoryReason(profile, category) {
    const continuousMinutes = continuousCapacityMinutes(profile);
...
function choosePlanType(profile, goal) {
    ...
    if (goal.goalDistance === "10K") {
        if ((goal.goalIntent === "finish" || goal.goalIntent === "finish_comfortably") &&
    ...
    if (goal.goalDistance === "Halvmaraton")
        return "HalfMarathon";
    return "Marathon";
}
