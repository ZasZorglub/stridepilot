...
const OUTPUT_DIR = process.env.BENCHMARK_EXPORT_DIR
    ...
    : path_1.default.join(process.cwd(), "exports", "benchmark-programmer-saet-4");
const dayLabels = {
    monday: "Mandag",
    tuesday: "Tirsdag",
    wednesday: "Onsdag",
...
function sessionCountProgression(plan) {
...
function roleMinutesByWeek(week) {
    return week.sessions.reduce((acc, session) => {
        acc[session.role] = (acc[session.role] ?? 0) + session.durationMin;
        return acc;
    }, {});
}
function roleDistribution(plan) {
...
function buildExportText(fixture, plan) {
    const lines = [];
    ...
    lines.push(`- Realistisk spænd: ${plan.timelineRecommendation.feasibleDurationRangeWeeks.minimum}-${plan.timelineRecommendation.feasibleDurationRangeWeeks.maximum} uger`);
    lines.push(`- Progressionstempo: ${progressionLabels[plan.timelineRecommendation.recommendedProgressionMode]}`);
    lines.push(`- Pas pr. uge: ${plan.timelineRecommendation.startingSessionsPerWeek} -> ${plan.timelineRecommendation.peakSessionsPerWeek}`);
    ...
    for (const week of plan.weeks) {
        ...
        lines.push(`Ugens mål: lang tur ${formatMinutes(week.longRunTargetMin)} · kontinuitet ${formatMinutes(plan.curves.continuousCurve[week.weekIndex - 1] ?? 0)} · volumen ${formatMinutes(week.volumeTargetMin)}`);
        for (const session of week.sessions) {
            ...
            if (session.coachingCues.length > 0)
                lines.push(`  Coaching cues: ${session.coachingCues.join(" / ")}`);
            if (session.notes && session.notes.length > 0)
                lines.push(`  Noter: ${session.notes.join(" / ")}`);
        }
...
for (const fixture of fixtures) {
    ...
    const filePath = path_1.default.join(OUTPUT_DIR, `${fixture.id}.txt`);
