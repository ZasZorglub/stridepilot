...
const OUTPUT_DIR = process.env.BENCHMARK_EXPORT_DIR
    ? path_1.default.resolve(process.cwd(), process.env.BENCHMARK_EXPORT_DIR)
    : path_1.default.join(process.cwd(), "exports", "stridepilot-5k-benchmark-set-1");
const dayLabels = {
    monday: "Monday",
    tuesday: "Tuesday",
    wednesday: "Wednesday",
...
function getIntervalType(session) {
    if (session.family === "intervals" || session.family === "fartlek" || session.family === "hill_reps")
        return session.family;
    if (session.family === "tempo_run" || session.family === "race_specific")
        return session.family;
    return null;
}
function getIntervalTotalTime(session) {
...
function biggestIncrease(values) {
    let maxIncrease = 0;
...
function buildJsonExport(fixture, plan) {
...
function buildTextSummary(fixture, plan) {
    ...
    json.weeklyProgression.forEach((week) => {
        lines.push(`- week ${week.weekNumber}: phase=${week.phase}, sessions=${week.sessions}, total=${formatMinutes(week.totalWeeklyTimeMin)}, long_run=${formatMinutes(week.longRunDurationMin)}, interval_type=${week.intervalType ?? "none"}, interval_total=${formatMinutes(week.intervalTotalTimeMin)}, easy_total=${formatMinutes(week.easyRunTotalTimeMin)}`);
    });
    lines.push("");
...
for (const fixture of fixtures) {
    ...
    const jsonPath = path_1.default.join(OUTPUT_DIR, `${fixture.fileStem}.json`);
    const textPath = path_1.default.join(OUTPUT_DIR, `${fixture.fileStem}.txt`);
    fs_1.default.writeFileSync(jsonPath, `${JSON.stringify(jsonExport, null, 2)}\n`, "utf8");
    fs_1.default.writeFileSync(textPath, textExport, "utf8");
    manifest.push(`${fixture.name}\nTXT: ${textPath}\nJSON: ${jsonPath}\n`);
    console.log(`${fixture.name} -> ${textPath}`);
}
fs_1.default.writeFileSync(path_1.default.join(OUTPUT_DIR, "README.txt"), manifest.join("\n"), "utf8");
