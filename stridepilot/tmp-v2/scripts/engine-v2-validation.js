...
function weekPicker(weeks) {
    const maxWeek = weeks.length;
    const indexes = new Set([1, Math.min(6, maxWeek), Math.min(10, maxWeek), maxWeek]);
    const specificWeek = weeks.find((week) => week.phase === "specific" || week.phase === "peak");
    if (specificWeek)
        indexes.add(specificWeek.weekIndex);
...
for (const fixture of fixtures) {
    ...
    console.log(`\n=== ${fixture.name} ===`);
    console.log(`classification=level:${plan.classification.traits.runnerLevel}, durability:${Math.round(plan.classification.traits.durabilityScore * 100)}, progression:${Math.round(plan.classification.traits.progressionTolerance * 100)}, intensity:${Math.round(plan.classification.traits.intensityReadiness * 100)}, longRun:${Math.round(plan.classification.traits.longRunReadiness * 100)}, consistency:${plan.classification.traits.consistencyProfile}, confidence:${plan.classification.traits.confidenceProfile}`);
    console.log(`planType=${plan.planTypeDecision.planType}`);
    console.log(`phases=${plan.phasePlan.blocks.map((block) => `${block.phase}[${block.startWeekIndex}-${block.endWeekIndex}] ${block.purpose.primaryObjective}`).join(" | ")}`);
    for (const week of chosenWeeks) {
        console.log(`U${week.weekIndex} | phase=${week.phase} | emphasis=${week.focus} | volume=${week.volumeTargetMin} | longRun=${week.longRunTargetMin} | intensity=${week.intensityTarget}`);
        for (const session of week.sessions) {
            console.log(`  - ${session.day} | ${session.role} | ${session.family} | ${session.summary} | ${session.estimatedTotalMinutes} min | purpose=${session.purpose}`);
        }
        console.log(`  issues=${week.validationIssues && week.validationIssues.length > 0 ? week.validationIssues.map((issue) => issue.message).join(" ; ") : "none"}`);
    }
}
