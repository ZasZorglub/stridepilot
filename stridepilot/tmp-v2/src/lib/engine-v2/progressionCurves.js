...
function longRunPeak(start, input) {
    if (input.raceDistance === "5K")
        return roundHalf(clamp(start + 12, 30, 75));
    if (input.raceDistance === "10K")
        return roundHalf(clamp(start + 24, 55, 95));
    if (input.raceDistance === "HalfMarathon")
        return roundHalf(clamp(start + 34, 85, 145));
    return roundHalf(clamp(start + 55, 120, 190));
}
function weeklyVolumeStart(input, longRunStartMin) {
...
function weeklyVolumePeak(start, input, classification) {
    const styleModifier = classification.traits.trainingStyle === "performance" ? 1.18 : classification.traits.trainingStyle === "conservative" ? 1.06 : 1.12;
...
function intensityForPhase(phase, planType, classification) {
    const base = phase === "base" ? 0.12 : phase === "build" ? 0.22 : phase === "specific" ? 0.36 : phase === "peak" ? 0.46 : 0.24;
...
function continuousRunTarget(input, phase, weekIndex, totalWeeks) {
    const current = input.currentContinuousRunMin;
    if (current >= 25 && input.goalType !== "return_to_running")
        return current;
    const peak = input.goalType === "return_to_running"
    ...
    if (phase === "taper")
        return roundHalf(Math.max(current, peak * 0.84));
...
function primaryDimension(phase, weekIndex, isCutback, intensity, classification) {
    if (isCutback)
        return "stabilize";
...
function growthCap(primary, previous, proposed) {
    const factor = primary === "long_run"
        ? 1.1
...
function buildProgressionCurves(input, classification, phasePlan, planType) {
