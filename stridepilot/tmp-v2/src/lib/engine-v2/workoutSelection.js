...
function roleForSlot(role, qualityBias, phase, goalType, backboneType) {
    if (role === "long_run") {
        if (qualityBias === "specific")
            return "long_with_segments";
        if (qualityBias === "short")
            return "long_short";
        return "long_run";
    }
    if (role === "recovery")
        return role;
    if (role === "easy")
        return qualityBias === "sharpen" ? "strides" : "easy";
    if (role === "aerobic_support") {
        if (backboneType === "continuous_backbone")
            return phase === "base" ? "support" : "steady";
        if (phase === "base")
            return "support";
        if (phase === "specific" || phase === "peak")
            return goalType === "target_time" || goalType === "improve_time" ? "steady" : "support";
        return "steady";
    }
    if (qualityBias === "short")
        return phase === "peak" ? "short_quality" : "race_pace_short";
    if (qualityBias === "specific")
        return goalType === "finish" || goalType === "finish_without_walking" ? "race_pace" : "intervals";
    if (qualityBias === "moderate")
        return goalType === "target_time" || goalType === "improve_time" ? "threshold" : "quality";
    if (qualityBias === "intro")
        return backboneType === "continuous_backbone" ? "quality" : "steady";
    return "quality";
}
function familyForSessionType(sessionType) {
    ...
    if (sessionType === "strides_session")
    ...
    if (sessionType === "recovery_jog")
        return "recovery_run";
    if (sessionType === "continuous_build" || sessionType === "development_run")
        return "development_run";
    return "long_run";
}
function getSessionType(params) {
    ...
    const finishGoal = goalType === "finish" || goalType === "finish_without_walking" || goalType === "build_consistency" || goalType === "return_to_running";
    const returnLike = goalType === "return_to_running";
    if (returnLike) {
        if (role === "recovery")
            return "recovery_jog";
        if (role === "strides")
            return returnToRunningState?.continuityGatePassed ? "strides_session" : "easy_run";
    ...
    if (backboneType === "continuous_backbone") {
        if (role === "long_run" || role === "long_short" || role === "long_with_segments") {
            ...
            if (role === "long_with_segments")
                return performanceGoal || phase === "specific" ? "long_run_with_blocks" : "long_run_progressive";
            if (beginner && (phase === "base" || phase === "build"))
                return "run_walk_long";
            if (beginner && finishGoal && phase === "specific")
                return "long_run_easy";
            if (beginner && finishGoal && phase === "taper")
                return "run_walk_long";
            return "long_run_easy";
        }
        if (role === "easy")
            return beginner && phase !== "specific" ? "run_walk" : "easy_run";
        if (role === "support")
            return "development_run";
        if (role === "steady")
            return beginner ? "continuous_build" : "steady_run";
        if (role === "quality")
            return beginner ? (phase === "base" ? "development_run" : "continuous_build") : "continuous_build";
        if (role === "threshold")
            return beginner ? "development_run" : "threshold_intervals";
        if (role === "intervals")
            return beginner ? "continuous_build" : performanceGoal ? "vo2_intervals" : "threshold_intervals";
        if (role === "race_pace" || role === "race_pace_short")
            return beginner ? "continuous_build" : performanceGoal ? "race_pace_blocks" : role === "race_pace_short" ? "progression_run" : "race_pace_blocks";
        if (role === "short_quality")
            return beginner ? "development_run" : "progression_run";
    }
    if (role === "long_run" || role === "long_short" || role === "long_with_segments") {
        if (role === "long_short" || phase === "taper")
            return "long_run_easy";
        if (role === "long_with_segments")
            return performanceGoal ? "long_run_with_blocks" : "long_run_progressive";
        if (advanced && phase === "specific")
            return "long_run_with_blocks";
        if ((runnerLevel === "intermediate" || advanced) && phase === "specific")
            return performanceGoal ? "long_run_with_blocks" : "long_run_progressive";
        if ((runnerLevel === "intermediate" || advanced) && phase === "peak")
            return "long_run_progressive";
        return "long_run_easy";
    }
    if (role === "easy")
        return "easy_run";
    if (role === "support")
        return advanced ? "steady_run" : runnerLevel === "recreational" || runnerLevel === "intermediate" ? "steady_run" : "development_run";
    if (role === "steady")
        return "steady_run";
    if (role === "quality") {
        if (phase === "peak" || phase === "taper")
            return performanceGoal ? "threshold_intervals" : "progression_run";
        if (advanced && performanceGoal && phase === "specific")
            return "vo2_intervals";
        if (runnerLevel === "recreational" || runnerLevel === "intermediate" || advanced)
            return "threshold_intervals";
        return finishGoal ? "development_run" : "continuous_build";
    }
    if (role === "short_quality")
        return goalType === "target_time" ? "vo2_intervals" : performanceGoal ? "threshold_intervals" : "progression_run";
    if (role === "threshold")
        return phase === "taper" ? (goalType === "target_time" ? "race_pace_blocks" : "progression_run") : "threshold_intervals";
    if (role === "intervals")
        return advanced || performanceGoal ? "vo2_intervals" : "threshold_intervals";
    if (role === "race_pace")
        return phase === "specific" || phase === "peak" || phase === "taper" || goalType === "target_time" ? "race_pace_blocks" : "progression_run";
    if (role === "race_pace_short")
        return goalType === "target_time" || goalType === "improve_time" ? "race_pace_blocks" : "progression_run";
...
function chooseWorkoutSelections(structure, phase, planType, goalType, classification, backboneType) {
    return structure.slots.map((slot, index) => {
        const sessionRole = roleForSlot(slot.role, slot.qualityBias, phase, goalType, backboneType);
