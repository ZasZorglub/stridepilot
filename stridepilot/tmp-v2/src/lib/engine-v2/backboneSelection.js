...
exports.selectBackbone = selectBackbone;
function selectBackbone(input, classification, goalClassification) {
    const reasons = [];
    ...
    const continuityLed10kFinish = input.raceDistance === "10K" &&
    ...
    if ((trueBeginner || veryLowContinuousCapacity || continuityFocusedGoal) && shortBeginnerGoal) {
        if (trueBeginner)
            reasons.push("Runneren er en ægte begynder, så progressionen skal styres af sammenhængende løbetid før andet.");
        if (veryLowContinuousCapacity)
            reasons.push("Den nuværende sammenhængende løbekapacitet er lav, så continuity er den mest stabile primære driver.");
        if (continuityFocusedGoal)
            reasons.push("Målet handler primært om kontinuitet og sikker opbygning frem for distance-specifik belastning.");
        return {
            type: "continuous_backbone",
            primaryMetric: "continuous_running",
            reasons,
    ...
    if (continuityLed10kFinish) {
        if (trueBeginner || beginnerPlus)
            reasons.push("Runneren er stadig tæt nok på beginner-niveau til, at 10K-forløbet skal bygges som en længere continuity-progression.");
        if (lowContinuousCapacity)
            reasons.push("Den sammenhængende løbekapacitet er stadig begrænset, så 10K-planen bør ligne et gradvist Couch-to-10K-forløb mere end et erfarent distanceprogram.");
