...
exports.classifyRunner = classifyRunner;
...
function hasReturnFlag(input) {
    return (input.goalType === "return_to_running" ||
        (input.freeTextFlags ?? []).some((flag) => /return|comeback|break|restart|back/i.test(flag)));
}
function capabilityScore(input) {
...
function consistencyScore(input) {
...
function consistencyProfile(score) {
    if (score < 0.3)
        return "sporadic";
    if (score < 0.55)
        return "developing";
...
function confidenceProfile(input, consistency, injuryRiskScore) {
    const confidenceValue = clamp((input.confidence ?? 3) / 5, 0, 1);
    if (input.currentContinuousRunMin < 5 || consistency < 0.3)
        return "fragile";
    if (confidenceValue < 0.45 || injuryRiskScore > 0.55)
        return "cautious";
    if (confidenceValue > 0.75 && consistency > 0.7)
        return "confident";
    return "stable";
}
function scheduleConstraintLevel(input) {
    const availability = input.availableTrainingDays.length;
    if (availability <= 2 || input.typicalAvailableTimeMin <= 35)
        return "high";
    if (availability === 3 || input.typicalAvailableTimeMin <= 50)
        return "moderate";
    return "low";
}
function injuryRiskScore(input, consistency) {
    const injury = input.injuryConcern === "high" ? 0.8 : input.injuryConcern === "moderate" ? 0.58 : input.injuryConcern === "low" ? 0.3 : 0.1;
    const externalLoad = input.externalTrainingLoad === "high" ? 0.65 : input.externalTrainingLoad === "moderate" ? 0.4 : input.externalTrainingLoad === "light" ? 0.2 : 0;
...
function primaryRunnerType(input, runnerLevel, capability, consistency, injury, schedule) {
    const returnFlag = hasReturnFlag(input);
    const performanceBias = input.trainingStylePreference === "performance" &&
    ...
    if (returnFlag && input.experienceLevel !== "none" && runnerLevel !== "advanced")
    ...
    if (runnerLevel === "beginner_plus")
        return "beginner_plus";
    if (injury >= 0.6)
        return "injury_sensitive";
    if (schedule === "high")
        return "low_availability_runner";
    if (consistencyGoal)
        return "consistency_builder";
    if (performanceBias && runnerLevel === "advanced")
        return "advanced_recreational";
    if (performanceBias)
...
function modifiers(input, consistency, injury, schedule, confidence, primaryType) {
    ...
    if (hasReturnFlag(input))
        result.add("return_from_break");
    if (input.goalType === "build_consistency" || primaryType === "consistency_builder")
        result.add("consistency_first");
    return [...result];
}
function deriveTraitScores(input, runnerLevel, capability, consistency, injury, schedule) {
    ...
    const durabilityScore = clamp(average([
        capability,
        ...
        injury * 0.25 -
        schedulePenalty * 0.25, 0, 1);
    const progressionTolerance = clamp(durabilityScore * 0.45 +
        consistency * 0.25 +
    ...
    const intensityReadiness = clamp(average([
        ...
        (runnerLevel === "true_beginner" ? 0.35 : runnerLevel === "beginner_plus" ? 0.16 : 0), 0, 1);
    const longRunReadiness = clamp(average([
    ...
    const recoveryNeed = clamp(0.22 +
        ...
        (runnerLevel === "true_beginner" ? 0.05 : 0), 0.16, 0.82);
    return {
        durabilityScore,
...
function capabilitySummary(input, runnerLevel, capability) {
...
function riskSummary(input, injury, schedule) {
...
function classificationSummary(primaryType, modifierList) {
