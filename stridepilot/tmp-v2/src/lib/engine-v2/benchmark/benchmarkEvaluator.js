...
exports.evaluateBenchmarkPlan = evaluateBenchmarkPlan;
exports.evaluateBenchmarkRunner = evaluateBenchmarkRunner;
exports.evaluateBenchmarkSuite = evaluateBenchmarkSuite;
const engine_1 = require("../engine");
function statusFromCounts(pass, borderline, fail) {
    if (fail > 0)
        return "out_of_band";
    if (borderline > 0 || pass === 0)
        return "borderline";
    return "within_band";
}
function noteResult(status, notes) {
    return { status, notes };
}
function inRange(value, band) {
    if (value >= band.min && value <= band.max)
        return "within_band";
    if ((band.borderlineMin === undefined || value >= band.borderlineMin) &&
        (band.borderlineMax === undefined || value <= band.borderlineMax)) {
        return "borderline";
    }
    return "out_of_band";
}
function phaseSequence(plan) {
    return plan.phasePlan.blocks.reduce((sequence, block) => {
        if (sequence.at(-1) !== block.phase)
            sequence.push(block.phase);
        return sequence;
    }, []);
}
function qualityFamilies(week) {
    return week.sessions.filter((session) => session.role === "quality").map((session) => session.family);
}
function roleSignature(week) {
    return week.sessions.map((session) => session.role).join(">");
}
function firstWeekMatching(plan, predicate) {
    return plan.weeks.find(predicate);
}
function weeksOfPhase(plan, phase) {
    return plan.weeks.filter((week) => week.phase === phase);
}
function adjustedTaperBand(reference, plan) {
...
function evaluatePhase(reference, plan) {
    const actual = phaseSequence(plan);
    const expected = reference.acceptance.expectedPhaseSequence;
    const notes = [];
    const missingRequired = expected.filter((phase) => !actual.includes(phase) && !(reference.acceptance.allowedPhaseOmissions ?? []).includes(phase));
...
function evaluateLongRun(reference, plan) {
    const notes = [];
    ...
    const finalLongRun = taperWeeks.at(-1)?.longRunTargetMin ??
    ...
    if (startStatus !== "within_band")
        notes.push(`Long-run start ${start} min is ${startStatus} vs expected ${reference.acceptance.longRunStartBand.min}-${reference.acceptance.longRunStartBand.max}.`);
    if (peakStatus !== "within_band")
        notes.push(`Long-run peak ${peak} min is ${peakStatus} vs expected ${reference.acceptance.longRunPeakBand.min}-${reference.acceptance.longRunPeakBand.max}.`);
    if (taperStatus !== "within_band")
        notes.push(`Pre-race taper reduction ${(taperReduction * 100).toFixed(0)}% is ${taperStatus}.`);
    const status = statusFromCounts([startStatus, peakStatus, taperStatus].filter((status) => status === "within_band").length, [startStatus, peakStatus, taperStatus].filter((status) => status === "borderline").length, [startStatus, peakStatus, taperStatus].filter((status) => status === "out_of_band").length);
    return noteResult(status, notes);
}
function evaluateWeeklyStructure(reference, plan) {
    const notes = [];
    ...
    const sampleWeeks = phaseExpectations && phaseExpectations.length > 0
        ? phaseExpectations
            ...
            .filter((entry) => Boolean(entry.week))
    ...
    for (const { week, expectation } of sampleWeeks) {
        ...
        if (qualityWeekFamilies.length > 0 &&
...
function evaluateIntensity(reference, plan) {
    const notes = [];
    ...
    const familyStatus = disallowed.length === 0 ? "within_band" : disallowed.length <= 2 ? "borderline" : "out_of_band";
    return noteResult(statusFromCounts([earlyStatus, peakStatus, familyStatus].filter((status) => status === "within_band").length, [earlyStatus, peakStatus, familyStatus].filter((status) => status === "borderline").length, [earlyStatus, peakStatus, familyStatus].filter((status) => status === "out_of_band").length), notes);
}
function milestoneWeekForContinuous(plan, targetMin) {
    return plan.curves.continuousCurve.findIndex((value) => value >= targetMin) + 1 || undefined;
}
function milestoneWeekForLongRun(plan, targetMin) {
    return plan.curves.longRunCurve.findIndex((value) => value >= targetMin) + 1 || undefined;
}
function milestoneWeekForPhase(plan, targetPhase) {
    return firstWeekMatching(plan, (week) => week.phase === targetPhase)?.weekIndex;
}
function evaluateMilestones(reference, plan) {
    const notes = [];
    let borderline = 0;
    let fail = 0;
    let pass = 0;
    for (const milestone of reference.acceptance.milestoneWindows) {
        let observed;
        if (milestone.label.includes("continuous") && milestone.targetMin)
            observed = milestoneWeekForContinuous(plan, milestone.targetMin);
        else if (milestone.label.includes("long_run") && milestone.targetMin)
            observed = milestoneWeekForLongRun(plan, milestone.targetMin);
        else if (milestone.targetPhase)
            observed = milestoneWeekForPhase(plan, milestone.targetPhase);
...
function evaluateRaceWeek(reference, plan) {
    const notes = [];
    const raceWeek = raceWeekOf(plan);
    if (!raceWeek)
        return noteResult("out_of_band", ["Plan has no final week."]);
    ...
    if (reference.acceptance.raceWeek.requireGoalSpecificSignal) {
        const hasGoalSignal = Boolean(raceEvent) ||
...
function scanFailureModes(reference, plan) {
    const warnings = [];
    const failures = [];
    ...
    for (const failureMode of reference.acceptance.failureModes) {
        if (failureMode === "beginner_stuck_in_tiny_run_walk") {
            ...
            if (lateRunWalk)
                failures.push("Beginner runner stays too locked in run/walk late in the plan.");
        }
            ...
            if (heavyFinishWeeks.length > 0)
                failures.push("Finish plan accumulates too much performance-like complexity.");
        }
        else if (failureMode === "marathon_too_generic") {
            if (engineIssues.has("One or more weeks make all sessions feel too similar."))
                failures.push("Marathon plan still looks too generic.");
        }
        else if (failureMode === "support_days_blur") {
            if ([...engineIssues].some((issue) => issue.includes("Support sessions blur together")))
                warnings.push("Support days still blur together too much.");
        }
        else if (failureMode === "long_run_underbuilt") {
            const peak = Math.max(...plan.curves.longRunCurve);
            if (peak < reference.acceptance.longRunPeakBand.min)
                failures.push(`Long-run peak ${peak} min is under the expected benchmark floor.`);
        }
        else if (failureMode === "no_real_taper" || failureMode === "no_taper") {
            if (!plan.weeks.some((week) => week.phase === "taper"))
                failures.push("Plan has no taper phase.");
        }
            ...
            if (plan.weeks[0]?.sessions.some((session) => session.family === "run_walk_progression"))
                failures.push("Non-beginner plan still opens with run/walk.");
        }
        else if (failureMode === "too_generic") {
            if (engineIssues.has("One or more weeks make all sessions feel too similar."))
                warnings.push("Plan still reads as too generic in some weeks.");
        }
        else if (failureMode === "no_specific_phase" || failureMode === "no_hm_specific_identity") {
            if (!plan.weeks.some((week) => week.phase === "specific" || week.phase === "peak"))
                failures.push("Plan lacks meaningful specific/peak identity.");
        }
            ...
            if (sharpWeeks.length > 1)
                warnings.push("Marathon improve uses too much sharp short-rep quality.");
        }
    }
    return {
...
function evaluateBenchmarkPlan(reference, plan) {
