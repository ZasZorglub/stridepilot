...
exports.renderBenchmarkReport = renderBenchmarkReport;
function renderBand(label, status) {
    return `${label}: ${status}`;
}
function renderFailure(scan) {
    if (scan.status === "pass")
        return "failureScan: pass";
...
function renderBenchmarkReport(suite) {
    const header = [
