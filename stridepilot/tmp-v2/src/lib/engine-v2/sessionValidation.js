...
exports.validateWeeklySessions = validateWeeklySessions;
exports.validateSessionSeries = validateSessionSeries;
function structureAllowed(family, structureType) {
    if (family === "long_run")
        return structureType === "long_run_continuous";
    if (family === "run_walk_progression")
        return structureType === "run_walk_blocks" || structureType === "long_run_run_walk";
    if (family === "recovery_run")
        return structureType === "continuous_easy";
    if (family === "easy_run")
        return structureType === "continuous_easy";
    if (family === "development_run")
        return structureType === "continuous_easy" || structureType === "development_blocks";
    if (family === "steady_run")
        return structureType === "continuous_steady";
    if (family === "tempo_run")
        return structureType === "tempo_block" || structureType === "tempo_intervals";
    if (family === "intervals" || family === "fartlek")
        return structureType === "interval_repeats";
    if (family === "progression_run")
        return structureType === "progression_blocks";
    if (family === "strides_session")
        return structureType === "strides_after_easy";
    if (family === "hill_reps")
        return structureType === "hill_repeat_structure";
    if (family === "race_specific")
        return structureType === "race_specific_blocks";
    return true;
}
function validateSession(session, selection, structure, classification, planType) {
    const issues = [];
    ...
    if (selection.role === "quality" && selection.intensityCap === "intro") {
        // no-op placeholder avoided; kept for future narrow selection checks
    }
    return issues;
}
function validateWeeklySessions(sessions, selections, structure, classification, planType) {
...
function validateSessionSeries(weeks, classification, planType) {
    const issues = [];
    if (classification.traits.runnerLevel === "true_beginner" || classification.traits.runnerLevel === "beginner_plus") {
    ...
    if (planType.includes("finish")) {
        const complexWeeks = weeks.filter((week) => week.sessions.filter((session) => session.family === "tempo_run" || session.family === "race_specific" || session.family === "intervals").length >= 2);
