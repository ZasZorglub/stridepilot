...
function rolesForFrequency(days, phase, planType, classification) {
    const beginner = classification.traits.runnerLevel === "true_beginner" || classification.traits.runnerLevel === "beginner_plus";
    if (days <= 2)
        return [beginner ? "easy" : phase === "base" ? "aerobic_support" : "quality", "long_run"];
    if (days === 3) {
        ...
        if (planType === "10k_finish")
            return [phase === "base" ? "easy" : "aerobic_support", "quality", "long_run"];
        return ["quality", "easy", "long_run"];
    }
    if (days === 4)
        return ["quality", phase === "base" ? "easy" : "aerobic_support", "recovery", "long_run"];
...
function buildWeeklyStructure(input, classification, phase, planType, weekIndex, curves) {
    ...
    const slots = roles.map((role, index) => {
        if (role === "long_run")
            return { role, day: longRunDay, qualityBias: "none", protected: true };
        const day = remainingDays[index] ?? availableDays[index] ?? longRunDay;
        const qualityBias = role === "quality"
            ? intensity < 0.22
                ? "intro"
                : intensity < 0.42
                    ? "moderate"
