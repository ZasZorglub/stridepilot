...
exports.predictRacePerformance = predictRacePerformance;
...
function runnerLevelMultiplier(level) {
...
function consistencyAdjustment(consistencyScore) {
    return clamp(1 - (consistencyScore - 0.5) * 0.16, 0.9, 1.1);
}
function loadAdjustment(weeklyLoad, sessionsPerWeek) {
...
function rpeAdjustment(recentRPE) {
    if (recentRPE == null)
        return 1;
    if (recentRPE <= 5)
        return 0.98;
...
function baseFiveKPaceSeconds(input) {
...
function raceTimeFromPace(distanceKm, secondsPerKm) {
    return roundSeconds(distanceKm * secondsPerKm);
}
function longerRacePace(basePace, multiplier, longRunDuration, weeklyLoad) {
...
function predictRacePerformance(input) {
