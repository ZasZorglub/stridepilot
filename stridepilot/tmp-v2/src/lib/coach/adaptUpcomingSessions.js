...
exports.adaptUpcomingSessions = adaptUpcomingSessions;
exports.summarizeNextWeekShift = summarizeNextWeekShift;
...
function normalizeAdaptiveDuration(seconds, min = 30, max = 4 * 60 * 60) {
    const raw = Number.isFinite(seconds) ? seconds : min;
    const clamped = Math.max(min, Math.min(max, raw));
    return Math.round(clamped / 30) * 30;
}
function inferDistance(plan) {
    const combined = plan.sessions.map((session) => `${session.title} ${session.notes ?? ""}`.toLowerCase()).join(" ");
    if (combined.includes("maraton"))
        return "Marathon";
    if (combined.includes("halvmaraton"))
        return "Halvmaraton";
    if (combined.includes("10 km"))
        return "10K";
    if (combined.includes("5k"))
        return "5K";
    const longestRunMinutes = Math.max(0, ...plan.sessions.map((session) => session.steps.filter((step) => step.type === "run").reduce((sum, step) => sum + step.durationSec / 60, 0)));
    if (longestRunMinutes >= 110)
        return "Marathon";
    if (longestRunMinutes >= 80)
        return "Halvmaraton";
    if (longestRunMinutes >= 55)
        return "10K";
    return "5K";
}
function classifySession(session) {
    const title = `${session.title} ${session.notes ?? ""}`.toLowerCase();
    if (/benchmark/.test(title))
        return "benchmark";
    if (/interval/.test(title))
        return "interval";
    if (/tempo/.test(title))
        return "tempo";
    if (/udholdenhed|lang|long/.test(title))
        return "long";
    if (/recovery/.test(title))
        return "recovery";
    if (/roligt|easy|strides/.test(title) || session.loadScore <= 5)
        return "easy";
    return "other";
}
function planSpecificity(plan, distance) {
...
function scaleRunSteps(session, factor, minimumRunMinutes = 8) {
    const next = {
        ...session,
        steps: session.steps.map((step) => ({ ...step })),
    };
        ...
        : sessionLabel.includes("run-walk")
            ...
            : sessionLabel.includes("interval")
                ...
                : sessionLabel.includes("tempo")
                    ? 4
                    : minimumRunMinutes;
    next.steps = next.steps.map((step) => step.type === "run"
        ? {
            ...step,
            durationSec: normalizeAdaptiveDuration(Math.max(factor < 1 ? Math.min(step.durationSec, minimumPerRunMinutes * 60) : minimumPerRunMinutes * 60, step.durationSec * factor)),
        }
        : step);
...
function softenQualityForDistance(session, distance, mode) {
    if (distance === "5K") {
        return capSessionRunMinutes(setSessionIdentity(session, {
            title: mode === "recovery_microcycle" ? "Roligt pas med lette strides" : "Roligt løb med strides",
            notes: mode === "recovery_microcycle"
...
function upgradeEasyToSpecific(session, distance, mode) {
    if (distance === "5K") {
        return capSessionRunMinutes(setSessionIdentity(session, {
...
function scaleWeek(sessions, params) {
    ...
    if (params.qualityStrategy === "convert") {
        ...
        if (qualityIndexes.length > 0) {
            const factor = params.qualityStrategy === "advance"
                ? params.distance === "5K"
                    ? 1.1
                    : params.distance === "Marathon"
                        ...
                        : 1.08
                : 1.04;
            nextSessions[qualityIndexes[0].index] = capSessionRunMinutes(scaleRunSteps(nextSessions[qualityIndexes[0].index], factor, 8), params.distance === "5K" ? 34 : params.distance === "Marathon" ? 48 : 40);
            nextSessions[qualityIndexes[0].index] = {
                ...nextSessions[qualityIndexes[0].index],
                notes: params.qualityStrategy === "advance"
                    ? params.distance === "5K"
...
function rebuildMicrocycle(plan, capability, mode) {
    const nextPlan = clonePlan(plan);
    if (nextPlan.sessions.length === 0)
        return nextPlan;
    ...
    if (mode === "down_shift") {
        weeks.set(nextWeek, scaleWeek(weeks.get(nextWeek) ?? [], {
            distance,
            baselineSpecificity,
    ...
    if (mode === "recovery_microcycle") {
        ...
        if (followingWeek !== undefined) {
            weeks.set(followingWeek, scaleWeek(weeks.get(followingWeek) ?? [], {
                distance,
                baselineSpecificity,
...
function adaptUpcomingSessions(plan, capability, decision) {
    const normalizedDecision = decision;
    const mode = normalizedDecision && "mode" in normalizedDecision
...
function summarizeNextWeekShift(before, after) {
