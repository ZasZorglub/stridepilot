...
exports.segmentHeartRateGuidance = segmentHeartRateGuidance;
exports.mergeAdjacentWorkoutSteps = mergeAdjacentWorkoutSteps;
...
function segmentToStepType(segment, index, structure) {
    ...
    if (segment.type === "recovery") {
        ...
        if (label.includes("opvarmning"))
            return "warmup";
        if (label.includes("ned") || label.includes("afslutning"))
            return "cooldown";
        ...
        if (!hasEarlierWork && hasLaterWork)
            return "warmup";
        if (hasEarlierWork && !hasLaterWork)
            return "cooldown";
    }
...
function segmentHeartRateGuidance(segment) {
    if (segment.type === "walk")
        return undefined;
    if (segment.type === "warmup") {
        return {
...
function sessionAwareHeartRateGuidance(segment, sessionType) {
    if (segment.type === "walk")
        return undefined;
...
function mergedStepLabel(type, left, right) {
    if (left === right)
        return left;
    if (type === "warmup")
        return "Opvarmning";
...
function mergeAdjacentWorkoutSteps(steps) {
    return steps.reduce((merged, step) => {
...
function expandStructure(structure, sessionType) {
    const steps = [];
