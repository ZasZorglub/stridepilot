...
exports.buildPlanRationale = buildPlanRationale;
exports.buildWeekRationales = buildWeekRationales;
exports.buildWorkoutRationales = buildWorkoutRationales;
exports.generatePlanExplanation = generatePlanExplanation;
exports.explainPlanAdjustment = explainPlanAdjustment;
exports.buildAdaptationRationale = buildAdaptationRationale;
exports.buildFeedbackResponseCopy = buildFeedbackResponseCopy;
function abilityLine(profile) {
...
function safetyLine(profile, plan) {
...
function weekShape(week, totalWeeks) {
    if (week.isStabilizationWeek)
        return "stabilize";
    if (week.weekNumber >= totalWeeks - 1 || week.phase === "race_preparation" || week.phase === "taper")
        return "taper";
    return "build";
}
function weekSummary(goal, week) {
    if (week.isStabilizationWeek) {
...
function workoutSummary(session, goal) {
    ...
    if (session.type === "tempo") {
        return {
            sessionId: session.id,
            summary: goal.goalDistance === "Halvmaraton" || goal.goalDistance === "Marathon"
...
function buildWeekRationales(plan) {
...
function buildWorkoutRationales(plan) {
    return plan.sessions.map((session) => workoutSummary(session, plan.goal));
}
function generatePlanExplanation(profile, plan) {
...
function explainPlanAdjustment(adjustment) {
...
function buildAdaptationRationale(params) {
    ...
    if (afterLoad !== beforeLoad) {
        changeSummary.push(afterLoad < beforeLoad
    ...
    if (longestAfter !== longestBefore) {
        changeSummary.push(longestAfter < longestBefore
    ...
    const runnerFocus = mode === "recovery_microcycle"
...
function negativeFeedback(input) {
    return (input.completionPct < 95 ||
...
function defaultAdjustmentForMode(rationale) {
...
function progressionPreviewForMode(rationale) {
    if (rationale.mode === "recovery_microcycle") {
...
function buildFeedbackResponseCopy(params) {
    ...
    let adjustmentExplanation = rationale.changeSummary.find((line) => (caution ? !positiveReadinessLanguage(line) : true)) ?? defaultAdjustmentForMode(rationale);
