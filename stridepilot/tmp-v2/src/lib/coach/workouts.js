...
function isConservativeGoalDayProfile(context) {
    const category = runnerCategory(context.profile);
...
function goalDayRunTarget(context) {
...
function buildGoalDayWorkout(context, type) {
    ...
    return baseSession(context, type, `${distanceLabel(context.goal)} måldag`, "Selve måldagen, hvor planen kulminerer i en samlet indsats, der matcher distancen og den opbygning du har lavet.", `Lade måldagen være en troværdig kulmination mod ${distanceLabel(context.goal)} frem for bare endnu et kort taper-pas.`, "Start kontrolleret, find rytmen tidligt, og lad indsatsen udvikle sig roligt gennem dagen.", structure);
}
...
function buildRaceSpecificWorkout(context) {
    if (context.isGoalSession)
        return buildGoalDayWorkout(context, "race-specific");
...
function buildBenchmarkWorkout(context) {
    if (context.isGoalSession)
        return buildGoalDayWorkout(context, "benchmark");
