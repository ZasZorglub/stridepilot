...
exports.decideAdaptationMode = decideAdaptationMode;
function recentWindow(history, size) {
    return history.slice(-Math.max(0, Math.min(6, size)));
}
function count(items, predicate) {
    return items.filter(predicate).length;
}
function bounceBackReady(items) {
    if (items.length < 2)
        return false;
...
function decideAdaptationMode(capability) {
    ...
    const tooEasyCount = count(focus, (item) => item.completed && item.completionPct >= 90 && item.difficulty === "easy" && item.energy === "high" && item.pain !== "moderate" && item.pain !== "high");
    const latestStable = recentWindow(recent, 2);
    const latestThree = recentWindow(recent, 3);
    const priorThree = recent.slice(0, -2).slice(-3);
    const hadRecentCautionMode = capability.lastAdaptationMode === "down_shift" ||
    ...
    const priorPatchWasRough = priorThree.length > 0 &&
        priorThree.some((item) => !item.completed ||
