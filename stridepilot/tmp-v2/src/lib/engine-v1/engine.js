...
function isBeginnerStyleGoal(goalType) {
...
function chooseBackboneType(profile) {
    ...
    if (profile.goalDistance === "5k" && profile.runnerLevel === "beginner_plus" && profile.goalType === "finish")
        return "hybrid";
    return "long_run";
}
...
function continuousTargetForDistance(goalDistance, goalType) {
    ...
    if (goalDistance === "5k")
        return isPerformanceGoal(goalType) ? 40 : 30;
    if (goalDistance === "10k")
        return 60;
...
function longRunPeakForDistance(goalDistance) {
    if (goalDistance === "5k")
        return 45;
    if (goalDistance === "10k")
        return 70;
...
function buildContinuousCurve(startContinuous, targetContinuous, weeks) {
...
function distributeVolumeAcrossWeek(weeklyVolume, longRun, intervalVolume, sessionsPerWeek) {
...
function assignDays(workouts, profile) {
    ...
    const ordered = [...workouts].sort((a, b) => {
        if (a.workoutType === "long")
            return 1;
        if (b.workoutType === "long")
            return -1;
        if (a.workoutType === "recovery" && b.workoutType !== "recovery")
            return 1;
        if (b.workoutType === "recovery" && a.workoutType !== "recovery")
            return -1;
        return 0;
    });
...
function buildTrainingPlan(profile) {
    ...
    const startLongRun = roundHalf(clamp(Math.max(profile.longestRecentRunMin * 0.9, profile.currentContinuousMin * 1.2), profile.goalDistance === "5k" ? 15 : 20, profile.longestRecentRunMin * 1.1));
    ...
    const weeks = Array.from({ length: profile.timelineWeeks }, (_, index) => {
        ...
        if (phase === "base" && !isPerformanceGoal(profile.goalType)) {
            distribution = distribution.map((item, itemIndex) => item.workoutType === "intervals" || item.workoutType === "tempo"
        ...
        if (phase === "race_week") {
            distribution = distribution.slice(0, Math.min(distribution.length, 3)).map((item, itemIndex) => {
                if (itemIndex === distribution.length - 1)
                    return { workoutType: "race_pace", durationMin: Math.max(16, item.durationMin * 0.7) };
