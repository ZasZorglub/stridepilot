...
exports.buildPhases = buildPhases;
exports.buildTaperWeeks = buildTaperWeeks;
exports.buildStepBackWeeks = buildStepBackWeeks;
exports.buildContinuousCurve = buildContinuousCurve;
exports.buildLongRunCurve = buildLongRunCurve;
exports.buildIntervalCurve = buildIntervalCurve;
exports.buildWeeklyVolumeCurve = buildWeeklyVolumeCurve;
exports.distributeVolumeAcrossWeek = distributeVolumeAcrossWeek;
exports.generateWorkout = generateWorkout;
exports.applySafetyRules = applySafetyRules;
exports.buildTrainingPlan = buildTrainingPlan;
...
function chooseBackboneType(profile) {
    if (profile.goalType === "return_to_running")
        return "hybrid";
    if (profile.runnerLevel === "true_beginner" || profile.runnerLevel === "beginner" || profile.goalType === "finish_without_walking")
        return "continuous";
    if (profile.runnerLevel === "beginner_plus" && profile.goalDistance !== "marathon" && isFinishGoal(profile.goalType))
        return "hybrid";
    return "long_run";
}
function buildPhases(timelineWeeks) {
...
function continuousTargetForDistance(goalDistance, goalType) {
    if (goalType === "finish_without_walking")
        return goalDistance === "10k" ? 60 : 30;
    if (goalDistance === "5k")
        return isPerformanceGoal(goalType) ? 42 : 30;
    if (goalDistance === "10k")
        return 62;
    if (goalDistance === "half_marathon")
        return 92;
    return 120;
}
function longRunPeakForDistance(goalDistance, goalType) {
    if (goalDistance === "5k")
        return goalType === "target_time" ? 50 : 45;
    if (goalDistance === "10k")
        return goalType === "target_time" ? 78 : 72;
    if (goalDistance === "half_marathon")
        return goalType === "target_time" ? 120 : 105;
    return goalType === "target_time" ? 180 : 165;
}
function levelContinuousStep(runnerLevel) {
    switch (runnerLevel) {
        case "true_beginner":
...
function buildContinuousCurve(startContinuous, targetContinuous, weeks, runnerLevel) {
...
function buildLongRunCurve(startLongRun, peakLongRun, weeks, stepBackWeeks, taperWeeks) {
    const totalWeeks = Math.max(4, weeks);
...
function intervalPhaseRatio(phase, goalType) {
...
function buildIntervalCurve(phases, weeklyVolumeCurve, runnerLevel, goalType, stepBackWeeks, taperWeeks) {
    const stepBacks = new Set(stepBackWeeks);
    const tapers = new Set(taperWeeks);
    const curve = [];
    let previous = 0;
    const levelFactor = runnerLevel === "true_beginner" || runnerLevel === "beginner"
        ? 0.4
    ...
    phases.forEach((phase, index) => {
        ...
        if (isFinishGoal(goalType))
            current = roundHalf(current * 0.6);
        previous = Math.max(0, current);
        curve.push(previous);
    });
    return curve;
}
function easyRecoveryVolumeForWeek(weeklyVolume, longRun, intervalVolume, goalType) {
...
function buildWeeklyVolumeCurve(longRunCurve, intervalCurve, profile) {
...
function choosePrimaryQualityType(goalType, phase) {
    if (phase === "race_week")
        return "race_pace";
    if (goalType === "target_time")
        return phase === "specific" || phase === "peak" ? "race_pace" : "intervals";
    if (goalType === "improve_time")
        return phase === "specific" ? "tempo" : "intervals";
    return "easy";
}
function distributeVolumeAcrossWeek(weeklyVolume, longRun, intervalVolume, sessionsPerWeek, phase, goalType) {
...
function generateWorkout(workoutType, duration) {
...
function assignDays(workouts, profile) {
    ...
    if (long)
        long.day = longRunDay;
    const dayIndex = (day) => day ? defaultDays.indexOf(day) : -1;
    ...
    for (const workout of workouts) {
        ...
        if (isQuality) {
            ...
            if (longIdx > 0)
                disallowed.add(defaultDays[longIdx - 1]);
            qualityDays.forEach((day) => {
                const idx = dayIndex(day);
                if (idx >= 0 && idx < defaultDays.length - 1)
                    disallowed.add(defaultDays[idx + 1]);
            });
        }
        ...
        if (isQuality && candidate)
            qualityDays.add(candidate);
    }
    return workouts;
}
function applySafetyRules(trainingPlan) {
    ...
    for (let index = 0; index < weeks.length; index += 1) {
        ...
        if (share > longRunCap)
            week.longRun = roundHalf(week.weeklyVolume * longRunCap);
        if (share < longRunFloor)
            week.longRun = roundHalf(week.weeklyVolume * longRunFloor);
...
function buildTrainingPlan(profile) {
    ...
    const startLongRun = roundHalf(clamp(Math.max(profile.longestRecentRunMin * 0.88, profile.currentContinuousMin * 1.15), profile.goalDistance === "5k" ? 15 : 20, profile.longestRecentRunMin * 1.05));
    ...
    const weeks = Array.from({ length: profile.timelineWeeks }, (_, index) => {
        const weekNumber = index + 1;
        ...
        if (phase === "base" && isFinishGoal(profile.goalType)) {
            distribution = distribution.map((item) => item.workoutType === "intervals" || item.workoutType === "tempo" || item.workoutType === "race_pace"
        ...
        if (phase === "race_week") {
            distribution = distribution.slice(0, Math.min(distribution.length, 3)).map((item, itemIndex, items) => {
                if (itemIndex === items.length - 1)
                    return { workoutType: isPerformanceGoal(profile.goalType) ? "race_pace" : "easy", durationMin: Math.max(16, item.durationMin * 0.6) };
