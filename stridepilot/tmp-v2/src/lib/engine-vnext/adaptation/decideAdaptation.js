...
exports.decideVNextAdaptation = decideVNextAdaptation;
function complianceRate(feedback) {
    if (feedback.sessionsPlanned <= 0)
        return 0;
    return feedback.sessionsCompleted / feedback.sessionsPlanned;
}
function nearestBoundaryWeekIndex(plan, weekIndex) {
...
function nearestRecoveryCandidate(plan, weekIndex) {
...
function reasonedDecision(action, feedback, conservativeBias, reasonCodes, boundaryWeekIndex) {
    return {
        action,
        reasonCodes,
...
function decideVNextAdaptation(plan, feedback) {
    ...
    if (!week || !hooks || !plan.adaptationHooks?.adaptationReady) {
        return reasonedDecision("keep_current_progression", feedback, conservativeBias, ["boundary_limited"], boundaryWeekIndex);
    }
    if (week.isRaceWeek || week.phase === "taper" || hooks.progressionGate === "restricted") {
        const reasonCodes = ["restricted_phase"];
        if (hooks.progressionGate === "restricted")
            reasonCodes.push("progression_gate_hold");
    ...
    if (feedback.painFlag) {
        const reasonCodes = ["pain_flag"];
        if (conservativeBias)
            reasonCodes.push("protected_runner_bias");
    ...
    if (feedback.fatigue === "high" && (hooks.recoveryMicrocycleCandidate || recoveryWeekIndex !== undefined)) {
        const reasonCodes = ["high_fatigue", "recovery_candidate_available"];
        if (conservativeBias)
            reasonCodes.push("protected_runner_bias");
        return reasonedDecision("insert_recovery_microcycle", feedback, conservativeBias, reasonCodes, recoveryWeekIndex ?? boundaryWeekIndex);
    }
    const lowComplianceThreshold = conservativeBias ? 0.75 : 0.65;
    if (compliance < lowComplianceThreshold) {
        const reasonCodes = ["low_compliance"];
        if (conservativeBias)
            reasonCodes.push("protected_runner_bias");
    ...
    if (hooks.progressionGate === "hold" || feedback.fatigue === "moderate" || feedback.confidence === "low") {
        return reasonedDecision("repeat_current_week", feedback, conservativeBias, ["progression_gate_hold"], boundaryWeekIndex);
    }
    return reasonedDecision("keep_current_progression", feedback, conservativeBias, ["progression_gate_open"], boundaryWeekIndex);
}
