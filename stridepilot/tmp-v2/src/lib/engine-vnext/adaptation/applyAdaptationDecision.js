...
exports.applyVNextAdaptationDecision = applyVNextAdaptationDecision;
const buildAdaptationHooks_1 = require("./buildAdaptationHooks");
...
function cloneSessionForWeek(session, targetWeekIndex, dayShift) {
    return {
        ...session,
...
function cloneWeekIntoTarget(sourceWeek, targetWeek) {
...
function noOp(plan, reason, sourceWeekIndex, targetWeekIndex) {
    return {
        plan,
        mutation: {
            applied: false,
...
function validateMutatedPlan(originalPlan, mutatedPlan, reason, sourceWeekIndex, targetWeekIndex) {
    mutatedPlan.vNextValidation = (0, validatePlan_1.validateVNextPlan)(mutatedPlan);
    mutatedPlan.adaptationHooks = (0, buildAdaptationHooks_1.buildAdaptationHooks)(mutatedPlan);
    const criticalIssues = (0, validation_1.validateEnginePlan)(mutatedPlan).filter((issue) => issue.severity === "critical");
    if (criticalIssues.length > 0 || (mutatedPlan.vNextValidation?.hardFailCount ?? 0) > 0) {
        return noOp(originalPlan, reason, sourceWeekIndex, targetWeekIndex);
    }
    return undefined;
}
function clonePlanWithWeek(plan, targetWeekIndex, replacement) {
    return {
        ...plan,
        weeks: plan.weeks.map((week) => (week.weekIndex === targetWeekIndex ? replacement : week)),
    };
}
function buildDownshiftedSession(session, targetWeekIndex, conservativeBias) {
...
function downshiftWeek(targetWeek, conservativeBias) {
...
function buildRecoverySession(session, targetWeekIndex, conservativeBias) {
...
function recoveryMicrocycleWeek(targetWeek, conservativeBias) {
...
function applyRepeatCurrentWeek(plan, decision) {
    ...
    const invalid = validateMutatedPlan(plan, mutatedPlan, "Repeat mutation was rolled back because post-mutation validation found critical issues.", sourceWeek.weekIndex, targetWeek.weekIndex);
    if (invalid)
        return invalid;
    return {
        plan: mutatedPlan,
...
function applyDownshiftNextWeek(plan, decision) {
    const sourceWeek = plan.weeks.find((week) => week.weekIndex === decision.targetWeekIndex);
    ...
    const invalid = validateMutatedPlan(plan, mutatedPlan, "Downshift mutation was rolled back because post-mutation validation found critical issues.", sourceWeek.weekIndex, targetWeek.weekIndex);
    if (invalid)
        return invalid;
    return {
        plan: mutatedPlan,
...
function applyRecoveryMicrocycle(plan, decision) {
    const sourceWeek = plan.weeks.find((week) => week.weekIndex === decision.targetWeekIndex);
    const recoveryCandidates = plan.adaptationHooks?.recoveryCandidateWeekIndices ?? [];
    const recoveryCandidateWeekIndex = recoveryCandidates.find((weekIndex) => weekIndex >= decision.targetWeekIndex) ??
        recoveryCandidates.find((weekIndex) => weekIndex >= decision.targetWeekIndex - 1);
    const targetWeekIndex = recoveryCandidateWeekIndex ??
    ...
    const invalid = validateMutatedPlan(plan, mutatedPlan, "Recovery microcycle mutation was rolled back because post-mutation validation found critical issues.", sourceWeek.weekIndex, targetWeek.weekIndex);
    if (invalid)
        return invalid;
    return {
        plan: mutatedPlan,
...
function applyVNextAdaptationDecision(plan, decision) {
