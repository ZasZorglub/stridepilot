...
exports.taperDistinctnessRule = exports.raceWeekDistinctnessRule = exports.beginnerReturnIntensityLockRule = exports.longRunShareSanityRule = void 0;
...
function longRunShareThresholds(goalType, runnerLevel) {
    if (goalType === "return_to_running")
        return { warning: 0.32, hardFail: 0.38 };
    if (runnerLevel === "true_beginner" || runnerLevel === "beginner_plus")
        return { warning: 0.35, hardFail: 0.4 };
    if (goalType === "target_time" || goalType === "improve_time")
        return { warning: 0.38, hardFail: 0.43 };
    if (runnerLevel === "advanced")
        return { warning: 0.42, hardFail: 0.48 };
    return { warning: 0.4, hardFail: 0.45 };
}
function qualityCount(week) {
    return week.sessions.filter((session) => !session.isRaceEvent && ["tempo_run", "intervals", "race_specific", "hill_reps", "progression_run"].includes(session.family)).length;
}
function complexityScore(week) {
...
exports.longRunShareSanityRule = {
    id: "ST-001",
    run: ({ plan }) => {
        ...
        return plan.weeks.flatMap((week) => {
            if (week.volumeTargetMin <= 0 || week.longRunTargetMin <= 0)
                return [];
...
exports.beginnerReturnIntensityLockRule = {
    id: "ST-002",
    run: ({ plan }) => {
        const beginnerLike = plan.classification.traits.runnerLevel === "true_beginner" ||
        ...
        if (!beginnerLike && !returnLike)
            return [];
        const illegalFamilies = returnLike ? ILLEGAL_RETURN_FAMILIES : ILLEGAL_BEGINNER_FAMILIES;
        return plan.weeks.flatMap((week) => week.sessions.flatMap((session) => {
            if (!illegalFamilies.has(session.family))
                return [];
            return [
                {
                    ruleId: "ST-002",
...
exports.raceWeekDistinctnessRule = {
    id: "ST-003",
    run: ({ plan }) => {
