import { Goal, GoalType, GuidancePreference, RunnerProfile } from "@/lib/types";

type ExperienceBand = "true_beginner" | "developing" | "experienced" | "returning";
...
export type ProfileSignals = {
  ...
  dislikesIntervals: boolean;
  fitButRunSpecificLow: boolean;
  otherTrainingLoad: "low" | "moderate" | "high";
};

export type TrainingDayRecommendation = {
...
function clampDays(value: number): 2 | 3 | 4 {
  if (value <= 2) return 2;
  if (value >= 4) return 4;
  return 3;
}
...
function goalTypeLabel(goalType?: GoalType, targetPaceSecPerKm?: number): string {
  if (goalType === "run_without_walking") return "løbe sammenhængende uden gangpauser";
  if (goalType === "target_time" && targetPaceSecPerKm) return `ramme omkring ${formatPace(targetPaceSecPerKm)}`;
  if (goalType === "pr" && targetPaceSecPerKm) return `sætte PR omkring ${formatPace(targetPaceSecPerKm)}`;
  if (goalType === "pr") return "jagte en personlig rekord";
  if (goalType === "target_time") return "løbe mod et konkret tempo";
  return "gennemføre målet på en god måde";
}

function guidanceLabel(preference?: GuidancePreference): string | null {
  if (preference === "simple") return "Du har også bedt om en enkel og overskuelig plan.";
  if (preference === "flexible") return "Planen er lagt, så den er lettere at tilpasse rundt om hverdagen.";
  if (preference === "performance_oriented") return "Planen er lagt lidt mere målrettet, fordi du gerne vil have tydelig fremgang.";
  return null;
}

export function interpretProfileSignals(runnerProfile: RunnerProfile, goal: Goal): ProfileSignals {
  const combinedText = [
  ...
  const limitedTime =
    hasKeyword(combinedText, ["travl", "lidt tid", "tidsmangel", "små børn", "kort tid"]) ||
  ...
  let experienceBand: ExperienceBand = "developing";
  if (
    explicitBeginner ||
    ...
    runnerProfile.runningExperience === "ovet" ||
  ...
  const fitButRunSpecificLow =
    (activityLevel === "høj" || activityLevel === "meget_høj") &&
    weeklyVolume <= 15 &&
    currentRunsPerWeek <= 2 &&
    longestRun <= 30;

  const otherTrainingLoad: "low" | "moderate" | "high" =
    hasKeyword(combinedText, ["crossfit", "cykler meget", "styrketræning 4", "mange kampe", "meget anden træning"])
      ? "high"
...
export function recommendTrainingDays(runnerProfile: RunnerProfile, goal: Goal): TrainingDayRecommendation {
  ...
  if (goal.goalType === "pr" || goal.goalType === "target_time") recommended = 4;
  if (signals.experienceBand === "true_beginner" || signals.injuryConcern) recommended = 3;
  if (signals.limitedTime && availability <= 2) recommended = 2;
  if (signals.otherTrainingLoad === "high" && recommended > 3) recommended = 3;
  if (currentRuns >= 4 || weeklyVolume >= 28) recommended = Math.max(recommended, 4);
  if (availability <= 2) recommended = 2;
  if (signals.prefersSimple && recommended > 3 && availability <= 3) recommended = 3;
...
export function buildProfileExplanationSummary(runnerProfile: RunnerProfile, goal: Goal): string[] {
  const signals = interpretProfileSignals(runnerProfile, goal);
