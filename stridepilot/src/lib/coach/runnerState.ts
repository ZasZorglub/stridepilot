...
export function createInitialRunnerState(capability: CapabilityState): RunnerState {
...
export function updateRunnerState(
  ...
  if (feedback.pain === "moderate") injuryRisk += 2;
  if (feedback.pain === "high") injuryRisk += 4;
  if (capability.recoveryDebt >= 6) injuryRisk += 1;
  if (capability.fatigueIndex >= 6) injuryRisk += 1;

  if (feedback.difficulty === "easy") confidence += 1;
  if (feedback.difficulty === "very_hard") confidence -= 1;
  if (feedback.energy === "high") confidence += 1;
  if (feedback.pain === "moderate") confidence -= 1;
  if (feedback.pain === "high") confidence -= 2;
