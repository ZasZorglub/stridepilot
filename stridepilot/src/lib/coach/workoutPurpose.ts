import { WorkoutSession as TrainingSession } from "@/lib/types";

type WorkoutPurposeKind = "easy" | "long" | "interval" | "tempo" | "recovery";

const PURPOSE_VARIANTS: Record<WorkoutPurposeKind, string[]> = {
  easy: [
...
function classifySession(session: TrainingSession): WorkoutPurposeKind {
  const text = `${session.title} ${session.notes ?? ""}`.toLowerCase();
  if (/interval|fartlek|bakke|hill/.test(text)) return "interval";
  if (/tempo|steady|progression|10 km-specifikt|race-specific/.test(text)) return "tempo";
  if (/recovery|restitution|hvile|roligt/.test(text)) return "recovery";
  if (/udholdenhed|lang|long/.test(text)) return "long";
  return "easy";
}

function hashSeed(session: TrainingSession, kind: WorkoutPurposeKind): number {
  const seed = `${session.id}|${session.week}|${kind}|${session.title}`;
  return Array.from(seed).reduce((sum, char) => sum + char.charCodeAt(0), 0);
}

export function getWorkoutPurpose(session: TrainingSession): string {
