import { CapabilityState, WorkoutFeedback, deriveRunnerTraits } from "./capability";
...
export function updateCapability(capability: CapabilityState, feedback: WorkoutFeedback): CapabilityState {
  ...
  if (feedback.completed) consistencyScore += 0.6;
  if (!feedback.completed) consistencyScore -= 1.4;

  if (feedback.difficulty === "easy" && feedback.completed) intervalTolerance += 0.4;
  if (feedback.difficulty === "hard") intervalTolerance -= 0.3;
  if (feedback.difficulty === "very_hard") intervalTolerance -= 0.6;
  if (feedback.pain === "moderate") intervalTolerance -= 0.5;
  if (feedback.pain === "high") intervalTolerance -= 1;

  if ((feedback.completionPct ?? 100) < 70) weeklyLoad *= 0.96;
  if ((feedback.adaptationFactor ?? 1) < 1) weeklyLoad *= feedback.adaptationFactor ?? 1;
  ...
  return {
    ...
    recentFeedback,
    traits: deriveRunnerTraits(recentFeedback),
