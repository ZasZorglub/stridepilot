...
const DECISION_VARIANTS: Record<CoachDecision["type"], string[]> = {
  progress: [
...
function hashSeed(input: CoachExplanationInput): number {
  const seed = [
    input.decision.type,
...
function pickVariant(input: CoachExplanationInput): string {
  const variants = DECISION_VARIANTS[input.decision.type];
  return variants[hashSeed(input) % variants.length];
}

function trendHint(trend?: TrainingTrend): string | null {
...
function blockHint(block?: TrainingBlock): string | null {
...
export function generateCoachExplanation(input: CoachExplanationInput): string {
