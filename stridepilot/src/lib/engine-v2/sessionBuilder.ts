import type {
  ...
  RunnerInput,
  SessionSegment,
  StructuredSessionDefinition,
  WarmupType,
  WorkoutFamily,
  WorkoutSelection,
} from "./models";
import { planStartWeekMonday } from "../calendar-week";
...
function dayOffset(day: BuiltSession["day"]): number {
  return ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"].indexOf(day);
}

function warmupSegments(type: WarmupType, quality: boolean): SessionSegment[] {
  if (type === "brisk_walk") return [{ kind: "warmup", label: "Rask gang", durationMin: quality ? 6 : 5 }];
  if (type === "walk_jog") {
    return [
...
function cooldownSegments(type: WarmupType): SessionSegment[] {
  if (type === "brisk_walk") return [{ kind: "cooldown", label: "Rolig gang", durationMin: 5 }];
  if (type === "walk_jog") {
    return [
...
function mergeConsecutiveSegments(segments: SessionSegment[]): SessionSegment[] {
...
function intensityMultiplier(level: SessionIntensityLevel): number {
  if (level === "very_easy") return 0.68;
  if (level === "easy") return 0.8;
  if (level === "steady") return 0.92;
  if (level === "moderate") return 1;
  if (level === "comfortably_hard") return 1.1;
  return 1.18;
}

function titleForFamily(family: WorkoutFamily): string {
  if (family === "easy_run") return "Roligt løb";
  if (family === "recovery_run") return "Recovery-løb";
  if (family === "development_run") return "Udviklingspas";
  if (family === "steady_run") return "Steady-pas";
  if (family === "tempo_run") return "Tempopas";
  if (family === "intervals") return "Intervalpas";
  if (family === "fartlek") return "Fartlek";
  if (family === "hill_reps") return "Bakkeintervaller";
  if (family === "progression_run") return "Progressionspas";
  if (family === "strides_session") return "Roligt løb med strides";
  if (family === "race_specific") return "Målspecifikt pas";
  if (family === "run_walk_progression") return "Run/walk-udvikling";
  return "Lang tur";
}

function raceTitleForDistance(raceDistance: RunnerInput["raceDistance"]): string {
  if (raceDistance === "5K") return "5 km måldag";
  if (raceDistance === "10K") return "10 km måldag";
  if (raceDistance === "HalfMarathon") return "Halvmaraton måldag";
  return "Maraton måldag";
}

function raceEventRunTarget(
  raceDistance: RunnerInput["raceDistance"],
...
function isBeginnerNoWalkRaceWeek(
  selection: WorkoutSelection,
  planType: PlanType,
  runnerLevel: RunnerClassification["traits"]["runnerLevel"],
): boolean {
  return (
    selection.isRaceWeek === true &&
...
function notesForSelection(selection: WorkoutSelection, planType: PlanType): string[] | undefined {
...
function beginnerProgressStage(targetContinuousMin: number): 1 | 2 | 3 | 4 {
  if (targetContinuousMin >= 14) return 4;
  if (targetContinuousMin >= 10) return 3;
  if (targetContinuousMin >= 7) return 2;
  return 1;
}

function runWalkRecipe(
  currentContinuousMin: number,
  targetContinuousMin: number,
  runnerLevel: RunnerClassification["traits"]["runnerLevel"],
  longRun: boolean,
  conservative: boolean,
  desiredTotalMinutes?: number,
): { runMin: number; walkMin: number; repeats: number; title: string; longRunContribution: number; structureType: SessionStructureType } {
  ...
  const walkMin =
    stage === 1 ? (cautious ? 2 : 1.5) : stage === 2 ? 1.5 : stage === 3 ? 1 : conservative ? 1 : 0.5;
  ...
  const repeats =
    stage >= 4
      ...
      : Math.max(
          longRun ? 4 : 4,
          Math.ceil((mainTarget + (longRun ? walkMin : 0)) / cycleDuration),
        );
  return {
...
function blueprintForSelection(
  selection: WorkoutSelection,
  phase: Phase,
  continuousMin: number,
  longRunMin: number,
  ...
  const warmupType =
    runnerLevel === "true_beginner" ? "brisk_walk" : runnerLevel === "beginner_plus" ? "walk_jog" : "easy_jog";
  ...
  if (selection.family === "easy_run") {
    return {
      ...
      warmupType,
      estimatedTotalMinutes: roundHalf(
        warmupDuration + cooldownDuration + Math.max(phase === "taper" ? 12 : 20, continuousMin * (phase === "taper" ? 0.46 : selection.conservative ? 0.82 : 0.9)),
      ),
      notes: selection.notes,
    };
  }
...
function buildMainSet(
  ...
  if (blueprint.structureType === "run_walk_blocks" || blueprint.structureType === "long_run_run_walk") {
    const recipe = runWalkRecipe(
      continuousMin * 0.6,
      continuousMin,
      runnerLevel,
      blueprint.structureType === "long_run_run_walk",
      selection.conservative,
      blueprint.structureType === "long_run_run_walk" ? longRunMin : undefined,
    );
    return {
  ...
  if (blueprint.structureType === "interval_repeats") {
    const progression = progressIntervalWorkout(
      { duration: Math.max(16, roundHalf(continuousMin * 0.12)) },
      selection.weekIndex,
      totalWeeks,
      phase,
      goalType,
...
function sumDuration(segments: SessionSegment[]): number {
  return roundHalf(
    segments.reduce((sum, segment) => sum + segment.durationMin * (segment.repeats ?? 1) + (segment.recoverMin ?? 0) * Math.max((segment.repeats ?? 1) - 1, 0), 0),
  );
}

function easyWarmup(runnerLevel: RunnerLevel, quality: boolean): number {
  if (runnerLevel === "true_beginner") return quality ? 6 : 5;
  if (runnerLevel === "beginner_plus") return quality ? 7 : 6;
  return quality ? 10 : 6;
}
...
function planProgress(weekNumber: number, totalWeeks: number): number {
  if (totalWeeks <= 1) return 1;
  return Math.max(0, Math.min(1, (weekNumber - 1) / (totalWeeks - 1)));
}

export function progressIntervalWorkout(
...
): { work: number; repeats: number; rest: number; label: string } {
...
export function progressTempoWorkout(
  workout: { duration: number },
  weekNumber: number,
  phase: Phase,
  goalType: GoalType,
): { blocks: Array<{ label: string; duration: number; repeats?: number; rest?: number }> } {
  const targetLike = goalType === "target_time";
  if (phase === "taper") {
...
export function progressLongRunWorkout(
  workout: { duration: number },
  weekNumber: number,
  phase: Phase,
  goalType: GoalType,
): {
  style: "easy" | "progressive" | "tempo_segments" | "race_pace_segments" | "short";
  blocks: Array<{ type: "easy" | "progression" | "threshold" | "race_pace"; duration: number; repeats?: number; rest?: number }>;
} {
  ...
  if (phase === "taper") return { style: "short", blocks: [{ type: "easy", duration: main }] };
  if (finishLike || phase === "base") return { style: "easy", blocks: [{ type: "easy", duration: main }] };
...
function buildEngineSession(
  ...
  const weekMonday = planStartWeekMonday(input.startDate);
...
export function buildSession(
  ...
  curves?: ProgressionCurves,
  selection?: WorkoutSelection,
): BuiltSession | StructuredSessionDefinition {
