...
function beginnerStage(continuousTargetMin: number, phase: Phase): 1 | 2 | 3 | 4 {
  if (phase === "taper" && continuousTargetMin >= 9) return 4;
  if (continuousTargetMin >= 14) return 4;
  if (continuousTargetMin >= 10) return 3;
  if (continuousTargetMin >= 7) return 2;
  return 1;
}
...
function familyForRole(
  ...
  weeklyEmphasis: WeeklyStructure["weeklyEmphasis"],
  ...
  if (role === "long_run") {
    if (beginner && (stage <= 3 || phase === "base")) return "run_walk_progression";
    return "long_run";
  }
  if (role === "recovery") return "recovery_run";

  if (planType === "5k_finish" || planType === "5k_finish_no_walk") {
    if (beginner) {
      if (role === "quality") {
        if (phase === "taper") return stage >= 3 ? "easy_run" : "development_run";
        if (stage >= 4) return phase === "specific" || phase === "peak" ? "development_run" : "easy_run";
        if (stage >= 2 && phase !== "base") return "development_run";
        return "run_walk_progression";
      }
      if (role === "easy") {
        if (phase === "taper") return stage >= 3 ? "easy_run" : "development_run";
        if (stage >= 4) return "easy_run";
        if (stage >= 3 || (stage === 2 && phase === "specific")) return "development_run";
        return "run_walk_progression";
      }
      return "run_walk_progression";
    }
    if (role === "quality") return phase === "specific" ? "development_run" : "easy_run";
    return "easy_run";
  }

  if (planType === "5k_improve" || planType === "5k_target_time") {
    if (role === "quality") {
      if (phase === "base") return "strides_session";
      if (phase === "build") return intensity < 0.32 ? "fartlek" : "hill_reps";
      if (phase === "specific") return planType === "5k_target_time" ? "intervals" : "tempo_run";
      if (phase === "peak") return "race_specific";
  ...
  if (planType === "10k_finish") {
    if (role === "quality") {
      if (phase === "base") return continuousTargetMin < 30 ? "development_run" : "steady_run";
      if (phase === "build") return weeklyEmphasis === "recovery_absorption" ? "development_run" : "steady_run";
      if (phase === "specific") return weeklyEmphasis === "recovery_absorption" ? "development_run" : "steady_run";
      if (phase === "peak") return "progression_run";
      return "development_run";
    }
    if (role === "aerobic_support") return phase === "taper" ? "easy_run" : "development_run";
    return "easy_run";
  }

  if (planType === "10k_improve" || planType === "10k_target_time") {
    if (role === "quality") {
      if (phase === "base") return "steady_run";
      if (phase === "build") return intensity >= 0.3 ? "tempo_run" : "fartlek";
      if (phase === "specific") return planType === "10k_target_time" ? "intervals" : "tempo_run";
      if (phase === "peak") return "race_specific";
      return "tempo_run";
    }
    return role === "aerobic_support" ? "steady_run" : "easy_run";
  ...
  if (planType === "hm_finish") {
    if (role === "quality") {
      if (phase === "base") return "steady_run";
      if (phase === "build") return weeklyEmphasis === "recovery_absorption" ? "development_run" : "steady_run";
      if (phase === "specific") return "progression_run";
      if (phase === "peak") return "steady_run";
      return "development_run";
    }
    if (role === "aerobic_support") return phase === "specific" ? "development_run" : "easy_run";
  ...
  if (planType === "hm_improve" || planType === "hm_target_time") {
    if (role === "quality") {
      if (phase === "base") return "steady_run";
      if (phase === "build") return "tempo_run";
      if (phase === "specific") return planType === "hm_target_time" ? "tempo_run" : "progression_run";
      if (phase === "peak") return "race_specific";
      return "steady_run";
    }
  ...
  if (planType === "marathon_finish" || planType === "marathon_improve" || planType === "marathon_target_time") {
    if (role === "quality") {
      if (phase === "base") return "steady_run";
      if (phase === "build") return "progression_run";
      if (phase === "specific") return "race_specific";
      if (phase === "peak") return planType === "marathon_target_time" ? "race_specific" : "progression_run";
      return "steady_run";
    }
    if (role === "aerobic_support") {
      if (phase === "base") return "development_run";
      if (phase === "build") return primaryLoadDimension === "long_run" ? "development_run" : "steady_run";
      if (phase === "specific" || phase === "peak") return "development_run";
      return "development_run";
    }
    return role === "easy" ? "easy_run" : "recovery_run";
  }
...
export function chooseWorkoutSelections(
  ...
  return structure.slots.map((slot) => {
    const family = familyForRole(
      slot.role,
      phase,
      planType,
      classification,
      intensity,
      structure.continuousTargetMin,
      structure.weeklyEmphasis,
