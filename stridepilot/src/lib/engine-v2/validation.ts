...
export function validateEnginePlan(plan: EnginePlan): ValidationIssue[] {
  ...
  if (plan.planTypeDecision.planType.includes("finish")) {
    const complexFinishWeeks = plan.weeks.filter((week) =>
      week.sessions.filter((session) => session.family === "tempo_run" || session.family === "intervals" || session.family === "race_specific").length >= 2,
    );
