import type { BackboneSelection, GoalClassification, LoadDimension, Phase, PhasePlan, PlanType, ProgressionCurves, RunnerClassification, RunnerInput } from "./models";
...
function cutbackEvery(planType: PlanType, runnerLevel: RunnerClassification["traits"]["runnerLevel"]): number {
  if (runnerLevel === "true_beginner") return 3;
  if (runnerLevel === "beginner_plus") return 3;
  if (runnerLevel === "advanced") return 4;
  return 3;
}

function planLongRunMinimum(planType: PlanType): number {
  if (planType === "5k_finish" || planType === "5k_finish_no_walk") return 15;
  if (planType === "5k_improve" || planType === "5k_target_time") return 22;
  if (planType === "10k_finish") return 35;
  if (planType === "10k_improve" || planType === "10k_target_time") return 42;
  if (planType === "hm_finish") return 62;
  if (planType === "hm_improve" || planType === "hm_target_time") return 75;
  if (planType === "marathon_finish") return 95;
  if (planType === "marathon_improve" || planType === "marathon_target_time") return 105;
  return 20;
}

function baselineLongRunPeak(planType: PlanType): number {
  if (planType === "5k_finish" || planType === "5k_finish_no_walk") return 35;
  if (planType === "5k_improve" || planType === "5k_target_time") return 45;
  if (planType === "10k_finish") return 70;
  if (planType === "10k_improve" || planType === "10k_target_time") return 80;
  if (planType === "hm_finish") return 110;
  if (planType === "hm_improve" || planType === "hm_target_time") return 130;
  if (planType === "marathon_finish") return 160;
  if (planType === "marathon_improve" || planType === "marathon_target_time") return 175;
  return 60;
}

function isTrueBeginnerNoWalk(classification: RunnerClassification, planType: PlanType): boolean {
  return classification.traits.runnerLevel === "true_beginner" && (planType === "5k_finish" || planType === "5k_finish_no_walk");
}

function isContinuityBackboneFinishPlan(backboneSelection: BackboneSelection, planType: PlanType): boolean {
  return backboneSelection.type === "continuous_backbone" && (planType === "5k_finish" || planType === "5k_finish_no_walk" || planType === "10k_finish");
}

function longRunPeakAdjustment(input: RunnerInput, classification: RunnerClassification): number {
  const runnerLevelAdjustment =
  ...
  const consistencyAdjustment =
    classification.traits.consistencyProfile === "high"
      ? 0.06
  ...
  const scheduleAdjustment =
    input.availableTrainingDays.length >= 5
      ? 0.05
  ...
  const styleAdjustment =
    classification.traits.trainingStyle === "performance"
      ? 0.05
      : classification.traits.trainingStyle === "conservative"
        ? -0.06
        : 0;
  const riskAdjustment =
    classification.traits.injuryRiskScore >= 55
      ? -0.12
...
function longRunStart(input: RunnerInput, classification: RunnerClassification, planType: PlanType): number {
  ...
  const consistentBackgroundFloor =
    classification.traits.consistencyProfile === "high" || classification.traits.consistencyProfile === "stable"
      ? input.longestRecentRunMin * 0.98
      : input.longestRecentRunMin * 0.92;
  const finishFriendlyCap =
    planType.includes("marathon")
      ? 140
...
function longRunPeak(start: number, input: RunnerInput, classification: RunnerClassification, planType: PlanType, totalWeeks: number): number {
...
function beginnerNoWalkCutbackWeeks(totalWeeks: number, taperWeeks: number): number[] {
  const activeWeeks = Math.max(totalWeeks - taperWeeks, 1);
  const weeks: number[] = [];
  if (activeWeeks >= 4) weeks.push(4);
  if (activeWeeks >= 7) weeks.push(7);
  if (activeWeeks >= 11) weeks.push(10);
  return weeks.filter((week) => week < activeWeeks);
}

function continuityBackboneCutbackWeeks(totalWeeks: number, taperWeeks: number, planType: PlanType): number[] {
  if (planType === "10k_finish") {
    const activeWeeks = Math.max(totalWeeks - taperWeeks, 1);
    const weeks: number[] = [];
    if (activeWeeks >= 4) weeks.push(4);
    if (activeWeeks >= 8) weeks.push(8);
    if (activeWeeks >= 12) weeks.push(12);
    return weeks.filter((week) => week < activeWeeks);
  }
  return beginnerNoWalkCutbackWeeks(totalWeeks, taperWeeks);
}

function beginnerSessionsPerWeekCurve(totalWeeks: number, taperWeeks: number): number[] {
  const finalTaperStart = Math.max(totalWeeks - taperWeeks + 1, totalWeeks - 1);
  return Array.from({ length: totalWeeks }, (_, index) => {
    const weekIndex = index + 1;
    if (weekIndex <= 2) return 2;
    if (weekIndex >= finalTaperStart) return 2;
    return 3;
  });
}

function continuityBackboneSessionsPerWeekCurve(totalWeeks: number, taperWeeks: number, planType: PlanType): number[] {
  if (planType === "10k_finish") {
    const finalTaperStart = Math.max(totalWeeks - taperWeeks + 1, totalWeeks - 1);
    return Array.from({ length: totalWeeks }, (_, index) => {
      const weekIndex = index + 1;
...
function smoothContinuousCurve(params: {
  ...
  if (taperWeeks <= 0) return curve;

  const taperCurve: number[] = [];
...
function buildIntervalDurationCurve(
  weeks: PhasePlan["weeks"],
  goalType: RunnerInput["goalType"],
  ...
  return weeks.map((week) => {
    if (week.isRaceWeek) return targetLike ? 2 : 3;
    if (week.phase === "taper") return targetLike ? 3 : 2;
    if (week.phase === "base") return beginner ? 1 : 2;
    if (week.phase === "build") {
      return week.phaseProgress >= 0.6 ? (improveLike || targetLike ? 5 : 4) : 3;
    }
    if (week.phase === "specific") {
      if (targetLike) return week.phaseProgress >= 0.6 ? 10 : 8;
      if (improveLike) return week.phaseProgress >= 0.6 ? 8 : 6;
      return 5;
    }
    if (week.phase === "peak") {
...
function weeklyVolumeStart(input: RunnerInput, longRunStartMin: number, planType: PlanType): number {
...
function weeklyVolumePeak(longRunPeakMin: number, input: RunnerInput, classification: RunnerClassification, planType: PlanType): number {
  const ratio =
    planType.includes("improve") || planType.includes("target_time")
      ...
      : input.raceDistance === "Marathon"
        ...
        : input.raceDistance === "HalfMarathon"
          ...
          : 2.7;
  const styleAdjustment =
    classification.traits.trainingStyle === "performance" ? 1.04 : classification.traits.trainingStyle === "conservative" ? 0.94 : 1;
  return roundHalf(clamp(longRunPeakMin * ratio * styleAdjustment, longRunPeakMin * 2.5, 420));
}

function loadGrowthRange(
  runnerLevel: RunnerClassification["traits"]["runnerLevel"],
  goalType: RunnerInput["goalType"],
): { minimum: number; maximum: number } {
  const performanceGoal = goalType === "improve_time" || goalType === "target_time";
  if (runnerLevel === "true_beginner") return { minimum: 0.05, maximum: 0.06 };
  ...
  if (runnerLevel === "intermediate") return { minimum: 0.06, maximum: 0.08 };
  return { minimum: 0.065, maximum: 0.08 };
}

function loadStepBackFactor(runnerLevel: RunnerClassification["traits"]["runnerLevel"]): number {
  if (runnerLevel === "true_beginner") return 0.82;
  if (runnerLevel === "beginner_plus" || runnerLevel === "recreational") return 0.8;
  if (runnerLevel === "intermediate") return 0.78;
  return 0.76;
}

function taperLoadFactors(
  runnerLevel: RunnerClassification["traits"]["runnerLevel"],
  totalTaperWeeks: number,
  taperWeekIndex: number,
): number {
  if (runnerLevel === "advanced" && totalTaperWeeks >= 3) {
    return [0.84, 0.7, 0.56][Math.min(taperWeekIndex, 2)] ?? 0.56;
  }
  if (totalTaperWeeks <= 1) return runnerLevel === "true_beginner" ? 0.64 : 0.68;
...
export function buildWeeklyLoadCurve(params: {
  weeks: PhasePlan["weeks"];
  startLoad: number;
  runnerLevel: RunnerClassification["traits"]["runnerLevel"];
  goalType: RunnerInput["goalType"];
...
function continuousRunPeak(input: RunnerInput, planType: PlanType): number {
  const current = input.currentContinuousRunMin;
  if (input.goalType === "return_to_running") return Math.max(18, current + 12);
  if (planType === "5k_finish" || planType === "5k_finish_no_walk") return 30;
  if (planType === "10k_finish") return 60;
  if (planType === "hm_finish") return 80;
  if (planType === "marathon_finish") return Math.max(current, 90);
  if (current <= 1) return input.raceDistance === "5K" ? 24 : input.raceDistance === "10K" ? 34 : 40;
  if (current <= 5) return input.raceDistance === "5K" ? 30 : input.raceDistance === "10K" ? 45 : 55;
  if (current <= 12) return input.raceDistance === "5K" ? 30 : input.raceDistance === "10K" ? 50 : 60;
  if (current >= 25) return current;
  return input.raceDistance === "5K" ? 30 : input.raceDistance === "10K" ? 50 : 70;
}

function buildContinuousMilestones(current: number, peak: number): number[] {
  if (current <= 1 && peak <= 30) return [2, 3, 5, 8, 10, 12, 15, 18, 22, 26, 30].filter((value) => value <= peak);
  const standard = [5, 10, 15, 20, 25, 30, 40, 50, 60, 80];
  const milestones = standard.filter((value) => value > current && value <= peak);
  if (milestones.length === 0 && peak > current) return [peak];
  if (milestones.at(-1) !== peak) milestones.push(peak);
  return milestones;
}

function continuousMilestoneFractions(current: number, milestoneCount: number): number[] {
  if (milestoneCount <= 1) return [0.72];
  if (current <= 1) return [0.08, 0.16, 0.24, 0.38, 0.5, 0.6, 0.7, 0.8, 0.88, 0.94, 0.98].slice(0, milestoneCount);
  if (current <= 5) return [0.12, 0.28, 0.44, 0.6, 0.76, 0.88].slice(0, milestoneCount);
  if (current <= 20) return [0.08, 0.18, 0.3, 0.42, 0.5, 0.72].slice(0, milestoneCount);
  return [0.12, 0.26, 0.42, 0.58, 0.74, 0.86].slice(0, milestoneCount);
}

function continuousRunTarget(input: RunnerInput, planType: PlanType, phase: Phase, weekIndex: number, totalWeeks: number, taperWeeks: number): number {
  const current = input.currentContinuousRunMin;
  ...
  milestones.forEach((milestone, index) => {
    ...
    if (weekIndex >= milestoneWeek) target = milestone;
  });

  if ((planType === "5k_finish" || planType === "5k_finish_no_walk") && current <= 1) return roundHalf(clamp(target, current, peak));
  if (phase === "taper") return roundHalf(Math.max(current, peak * 0.9, target));
...
function longRunCutbackFactor(planType: PlanType): number {
  if (planType === "5k_finish" || planType === "5k_finish_no_walk") return 0.88;
  if (planType.includes("marathon")) return 0.88;
  if (planType.includes("hm")) return 0.88;
  if (planType.includes("finish")) return 0.9;
  return 0.9;
}

function taperCurve(totalTaperWeeks: number, taperWeekIndex: number): { volumeFactor: number; longRunFactor: number } {
...
function raceWeekAdjustments(planType: PlanType): { volumeFactor: number; longRunFactor: number; intensityFactor: number } {
...
function primaryDimension(
  ...
  if (backboneSelection.type === "continuous_backbone") {
    if (phase === "base") return weekIndex % 2 === 0 ? "long_run" : "continuous_running";
    if (phase === "build") return weekIndex % 3 === 0 ? "long_run" : "continuous_running";
    if (phase === "specific") return "continuous_running";
    return "stabilize";
  }
  if (classification.traits.runnerLevel === "true_beginner" && (planType === "5k_finish" || planType === "5k_finish_no_walk")) {
    if (phase === "base") return weekIndex % 2 === 0 ? "long_run" : "continuous_running";
    if (phase === "build" || phase === "specific") return "long_run";
    return "stabilize";
  }
  if (planType === "marathon_finish") {
    if (phase === "base") return weekIndex <= 2 ? "weekly_volume" : "long_run";
    if (phase === "build" || phase === "specific" || phase === "peak") return "long_run";
    return "stabilize";
  }
  if (classification.traits.runnerLevel === "true_beginner" || classification.traits.runnerLevel === "beginner_plus") {
    ...
    if (phase === "build") return weekIndex % 2 === 0 ? "continuous_running" : "long_run";
    if (phase === "specific") return "long_run";
  }
...
function growthCap(primary: LoadDimension, previous: number, proposed: number, planType?: PlanType): number {
...
export function buildProgressionCurves(
  ...
  for (const week of phasePlan.weeks) {
    ...
    if (week.phase === "taper" && !week.isRaceWeek) taperWeeks.push(week.weekIndex);
