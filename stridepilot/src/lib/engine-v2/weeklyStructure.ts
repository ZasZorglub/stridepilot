import type {
  ...
  RunnerLevel,
  WeeklyEmphasis,
  WeeklyLoadDistributionEntry,
  WeeklyRole,
  WeeklyStructure,
} from "./models";
import { selectArchetype } from "../engine-vnext/archetypes/selectArchetype";
import { getTaperStrategy } from "../engine-vnext/phases/taperStrategies";
import { selectWeeklyTemplate } from "../engine-vnext/templates/selectWeeklyTemplate";
import type { PhaseType as VNextPhaseType, RunnerLevel as VNextRunnerLevel } from "../engine-vnext/types";
...
function mapRaceDistanceToVNext(raceDistance: RaceDistance): "5K" | "10K" | "HalfMarathon" | "Marathon" {
  return raceDistance;
}
...
function selectVNextStructureRoles(params: {
  ...
  runnerType: PrimaryRunnerType;
  goalType: GoalType;
  raceDistance?: RaceDistance;
  isRaceWeek?: boolean;
  isStepBackWeek?: boolean;
}): PhaseStructureRole[] {
  const phase: VNextPhaseType = params.isRaceWeek ? "race_week" : params.phase;
...
function mapPhaseStructureRole(
  role: PhaseStructureRole,
  sessionsPerWeek: number,
): { role: WeeklyRole; qualityBias?: "none" | "intro" | "moderate" | "specific" | "short" | "sharpen" } {
  if (role === "long_run") return { role: "long_run", qualityBias: "none" };
  if (role === "long_with_segments") return { role: "long_run", qualityBias: "specific" };
  if (role === "long_short") return { role: "long_run", qualityBias: "short" };
  if (role === "recovery") return { role: "recovery", qualityBias: "none" };
  if (role === "strides") return { role: "easy", qualityBias: "sharpen" };
  if (role === "easy") return { role: "easy", qualityBias: "none" };
  if (role === "support") return { role: "aerobic_support", qualityBias: "none" };
  if (role === "steady") return { role: "aerobic_support", qualityBias: sessionsPerWeek <= 2 ? "intro" : "none" };
  if (role === "short_quality" || role === "race_pace_short") return { role: "quality", qualityBias: "short" };
  if (role === "threshold") return { role: "quality", qualityBias: "moderate" };
  if (role === "intervals" || role === "race_pace") return { role: "quality", qualityBias: "specific" };
  return { role: "quality", qualityBias: sessionsPerWeek <= 2 ? "moderate" : "intro" };
}

interface GoalTypeAdjustedWeekPlan {
  sessionRoles: PhaseStructureRole[];
  qualityDensity: "low" | "moderate" | "high";
  longRunStyle: "easy" | "progressive" | "segmented" | "short";
...
export function adjustPlanByGoalType(
  plan: {
    phase: Phase;
    sessionRoles: PhaseStructureRole[];
    isRaceWeek?: boolean;
  ...
  let qualityDensity: GoalTypeAdjustedWeekPlan["qualityDensity"] = "moderate";
  let longRunStyle: GoalTypeAdjustedWeekPlan["longRunStyle"] = "easy";
  let intensityDistribution: GoalTypeAdjustedWeekPlan["intensityDistribution"] = { easy: 0.8, moderate: 0.15, hard: 0.05 };

  if (returnLike) {
    qualityDensity = "low";
    intensityDistribution = { easy: 0.92, moderate: 0.08, hard: 0 };
    sessionRoles = sessionRoles.map((role) => {
      if (role === "quality" || role === "intervals" || role === "threshold" || role === "race_pace") return "support";
      if (role === "race_pace_short" || role === "short_quality" || role === "strides") return "easy";
      return role;
    });
  ...
  if (finishLike) {
    ...
    if (plan.phase === "build" || plan.phase === "specific" || plan.phase === "peak") {
      sessionRoles = sessionRoles.map((role) => {
        if (role === "intervals" || role === "threshold" || role === "short_quality") return beginnerLike ? "support" : "steady";
        if (role === "quality") return beginnerLike ? "support" : "steady";
        if (role === "race_pace") return plan.phase === "specific" ? "steady" : "easy";
    ...
    if (sessionRoles.length >= 3) {
      sessionRoles = sessionRoles.map((role, index) => {
        if (index === sessionRoles.length - 1) return role;
  ...
  if (improveLike) {
    ...
    if (plan.phase === "build") {
      sessionRoles = sessionRoles.map((role, index) => {
        if (role === "steady" && index === 0) return "threshold";
        if (role === "support") return "steady";
        return role;
      });
    }
    if (plan.phase === "specific") {
      ...
      if (qualityIndexes[0]) sessionRoles[qualityIndexes[0].index] = "intervals";
      if (qualityIndexes[1]) sessionRoles[qualityIndexes[1].index] = "threshold";
    }
    if (plan.phase === "specific" && sessionRoles.includes("long_run")) {
    ...
    if (plan.phase === "peak") {
      sessionRoles = sessionRoles.map((role, index) => {
        if (index === 0 && (role === "race_pace" || role === "short_quality")) return "short_quality";
        if (index === 1 && role === "easy" && sessionRoles.length >= 4) return "easy";
        return role;
      });
      longRunStyle = "short";
    }
    if (plan.isRaceWeek && sessionRoles.length >= 2) {
      sessionRoles = sessionRoles.map((role, index) => {
        if (index === 1) return "race_pace_short";
        if (index === sessionRoles.length - 1) return "strides";
        return "easy";
      });
    }
    return { sessionRoles, qualityDensity, longRunStyle, intensityDistribution };
  ...
  if (targetLike) {
    ...
    if (plan.phase === "base") {
      sessionRoles = sessionRoles.map((role, index) => {
        if (role === "support" || role === "steady") return index === 0 ? "threshold" : "race_pace";
        if (role === "quality") return "threshold";
        return role;
      });
    }
    if (plan.phase === "build") {
      sessionRoles = sessionRoles.map((role, index) => {
        if (index === 0 && (role === "threshold" || role === "quality")) return "intervals";
        if ((role === "support" || role === "steady") && sessionRoles.length >= 4) return "race_pace";
        return role;
      });
    }
    sessionRoles = sessionRoles.map((role) => {
      if (role === "support" || role === "steady") return "race_pace";
      if (role === "quality" && plan.phase === "base") return "threshold";
      return role;
    });
    ...
    if (plan.phase === "specific") {
      ...
      if (qualityIndexes[0]) sessionRoles[qualityIndexes[0].index] = "intervals";
      if (qualityIndexes[1]) sessionRoles[qualityIndexes[1].index] = "race_pace";
    }
    if (plan.phase === "peak") {
      sessionRoles = sessionRoles.map((role, index) => {
        if (index === 0 && role !== "long_short") return "intervals";
        if (index === 1 && role === "easy" && sessionRoles.length >= 4) return "race_pace";
        return role;
      });
    }
    if (plan.isRaceWeek && sessionRoles.length >= 2) {
...
export function getWeeklyStructure(params: {
  phase: Phase;
  sessionsPerWeek: number;
  runnerType: PrimaryRunnerType;
  goalType: GoalType;
  ...
  const buildQuality: PhaseStructureRole = performanceLike ? "threshold" : beginnerLike && finishLike ? "steady" : "quality";

  let sessionRoles: PhaseStructureRole[];
...
export function assignSessionRoles(
  weeklyStructure: { sessionsPerWeek: number },
  phase: Phase,
  goalType: GoalType,
  runnerType: PrimaryRunnerType,
  raceDistance?: RaceDistance,
...
function baseRoleOrderForSessions(sessionsPerWeek: number): WeeklyRole[] {
  if (sessionsPerWeek <= 2) return ["easy", "long_run"];
  if (sessionsPerWeek === 3) return ["easy", "aerobic_support", "long_run"];
  if (sessionsPerWeek === 4) return ["easy", "aerobic_support", "recovery", "long_run"];
  return ["easy", "aerobic_support", "quality", "recovery", "long_run"];
}

function distributionWeightsForRoles(
  roles: WeeklyRole[],
  phase: Phase,
  runnerLevel: RunnerLevel,
): number[] {
  const baseWeights: Record<WeeklyRole, number> =
    phase === "base"
  ...
  if (runnerLevel === "true_beginner" || runnerLevel === "beginner_plus") {
    return adjustedWeights.map((weight, index) => {
      if (roles[index] === "long_run" && phase !== "taper") return weight + 0.03;
      if (roles[index] === "quality") return Math.max(0.08, weight - 0.04);
      if (roles[index] === "aerobic_support") return weight + 0.01;
      return weight;
    });
  }
...
function capLongRunShare(weights: number[], roles: WeeklyRole[], phase: Phase, runnerLevel: RunnerLevel): number[] {
  const longRunIndex = roles.findIndex((role) => role === "long_run");
  if (longRunIndex === -1) return weights;

  const cap =
    phase === "taper" || phase === "peak"
  ...
  if (longRunWeight <= cap) return adjusted;
  ...
  if (otherTotal <= 0) return normalizeWeights(adjusted);
...
export function distributeWeeklyLoad(params: {
  weeklyLoad: number;
  sessionsPerWeek: number;
  phase: Phase;
  runnerLevel: RunnerLevel;
  ...
  if (goalType === "finish" || goalType === "finish_without_walking" || goalType === "build_consistency" || goalType === "return_to_running") {
    weights = normalizeWeights(
      weights.map((weight, index) => {
        ...
        if (role === "quality") return weight * 0.65;
        if (role === "aerobic_support" || role === "easy") return weight * 1.12;
        if (role === "recovery") return weight * 1.08;
        return weight;
      }),
    );
    ...
    weights = normalizeWeights(
      weights.map((weight, index) => {
        const role = roles[index];
        if (role === "quality") return weight * 1.12;
        if (role === "long_run") return weight * (phase === "specific" ? 0.95 : 1);
        return weight;
      }),
    );
    ...
    weights = normalizeWeights(
      weights.map((weight, index) => {
        const role = roles[index];
        if (role === "quality") return weight * 1.2;
        if (role === "aerobic_support") return weight * 0.85;
        if (role === "long_run") return weight * (phase === "specific" || phase === "peak" ? 0.98 : 1);
        return weight;
      }),
    );
  }
...
function distributeLoadAcrossSlots(
  slots: Array<{ role: WeeklyRole; day: DayOfWeek; qualityBias?: "none" | "intro" | "moderate" | "specific" | "short" | "sharpen"; protected?: boolean }>,
  weeklyLoad: number,
  phase: Phase,
  runnerLevel: RunnerLevel,
  goalType: GoalType,
): Array<{ role: WeeklyRole; day: DayOfWeek; targetDurationMin: number; qualityBias?: "none" | "intro" | "moderate" | "specific" | "short" | "sharpen"; protected?: boolean }> {
  ...
  const availableByRole = new Map<WeeklyRole, number[]>();
...
export function buildWeeklyStructure(
  ...
  const weeklyVolumeTargetMin =
    phaseWeek.isRaceWeek && taperStrategy
      ...
      : phase === "taper" && taperStrategy
        ...
        : baseWeeklyVolume;
  const longRunTargetMin =
    phaseWeek.isRaceWeek
      ? 0
      : phase === "taper" && taperStrategy
        ? roundHalf(Math.min(baseLongRun, peakLongRun * (1 - taperStrategy.longRunReduction)))
        : baseLongRun;
  const returnToRunningState =
    input.goalType === "return_to_running" || classification.traits.primaryRunnerType === "return_to_running"
      ? {
          weekIndex,
