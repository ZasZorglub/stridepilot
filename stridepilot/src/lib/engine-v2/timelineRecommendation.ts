import { deriveCalendarWeekCount, parseIsoDateLocal } from "../calendar-week";
import type {
  GoalClassification,
  RecommendationProgressionMode,
...
function currentDurationWeeks(input: RunnerInput): number | undefined {
  ...
  if (input.goalDate) return deriveCalendarWeekCount(input.startDate, input.goalDate) ?? undefined;
  return undefined;
}

function baselineWeeks(input: RunnerInput): number {
  if (input.goalType === "return_to_running") return 10;
  if (input.goalType === "build_consistency") return 10;
  if (input.goalType === "finish_without_walking") return input.raceDistance === "5K" ? 8 : 16;
  if (input.goalType === "target_time") {
    if (input.raceDistance === "5K") return 10;
    if (input.raceDistance === "10K") return 13;
    if (input.raceDistance === "HalfMarathon") return 16;
    return 20;
  }
  ...
  if (input.raceDistance === "5K") return 10;
  if (input.raceDistance === "10K") return 15;
  if (input.raceDistance === "HalfMarathon") return 17;
  return 20;
}

function safeMinimumWeeks(input: RunnerInput, classification: RunnerClassification): number {
  ...
  if (input.goalType === "finish_without_walking") return beginner ? 10 : beginnerPlus ? 9 : 8;
  if (input.goalType === "return_to_running") return 8;
  if (input.goalType === "build_consistency") return 8;
  if (input.raceDistance === "5K") return beginner ? 8 : beginnerPlus ? 7 : 6;
  if (input.raceDistance === "10K") return beginner ? 12 : beginnerPlus ? 10 : 8;
  if (input.raceDistance === "HalfMarathon") return beginner ? 16 : 12;
  return beginner ? 20 : 16;
}

function maximumReasonableWeeks(input: RunnerInput, classification: RunnerClassification): number {
  if (input.goalType === "finish_without_walking") {
    if (input.raceDistance === "5K" && classification.traits.runnerLevel === "true_beginner") return 14;
    return 16;
  }
  if (input.goalType === "build_consistency") return 16;
  if (input.raceDistance === "5K") return 14;
  if (input.raceDistance === "10K") return 18;
  if (input.raceDistance === "HalfMarathon") return 20;
  return 24;
}

function classifyGoal(input: RunnerInput): GoalClassification {
...
function abilityAdjustment(input: RunnerInput, classification: RunnerClassification): number {
  const level = classification.traits.runnerLevel;
  let adjustment = 0;
  if (level === "true_beginner") adjustment += 2;
  if (level === "beginner_plus") adjustment += 1;
  if (level === "intermediate") adjustment -= 1;
  if (level === "advanced") adjustment -= 2;

  if (input.currentContinuousRunMin <= 5) adjustment += 1;
  else if (input.currentContinuousRunMin <= 12) adjustment += 1;
  else if (input.currentContinuousRunMin >= 45) adjustment -= 1;

  if (input.longestRecentRunMin >= 75) adjustment -= 1;

  return adjustment;
}

function consistencyRiskAdjustment(input: RunnerInput, classification: RunnerClassification): number {
  let adjustment = 0;
  if (classification.traits.consistencyProfile === "sporadic") adjustment += 1;
  else if (classification.traits.consistencyProfile === "developing") adjustment += 1;
  else if (classification.traits.consistencyProfile === "high") adjustment -= 1;

  if (classification.traits.injuryRiskScore >= 55) adjustment += 2;
  else if (classification.traits.injuryRiskScore >= 40) adjustment += 1;

  if (classification.traits.recoveryNeed >= 0.65) adjustment += 1;
  if (input.externalTrainingLoad === "high") adjustment += 1;

  return adjustment;
}

function availabilityAdjustment(input: RunnerInput, goal: GoalClassification): number {
  const days = input.availableTrainingDays.length;
  let adjustment = 0;
  if (days <= 2) adjustment += 2;
  else if (days === 3) adjustment += goal.demand === "performance_specific" ? 1 : 0;
  else if (days >= 5) adjustment -= 1;
...
function rangeWindow(input: RunnerInput, classification: RunnerClassification): { aggressive: number; gentle: number } {
...
): { mode: RecommendationProgressionMode; warnings: string[]; durationShift: number } {
...
): { start: number; recommended: number; peak: number } {
...
function realismBand(finalWeeks: number, recommendedWeeks: number, minimumWeeks: number, requestedWeeks?: number): RecommendationRealism {
  if (typeof requestedWeeks !== "number") return "high_confidence";
  if (requestedWeeks < minimumWeeks) return "capped";
  if (requestedWeeks < recommendedWeeks) return "stretch";
  if (requestedWeeks <= recommendedWeeks + 2) return "realistic";
  return "high_confidence";
}

export function buildTimelineRecommendation(input: RunnerInput, classification: RunnerClassification): {
  ...
  const recommendation: TimelineRecommendation = {
    ...
    explanation: {
      ...
      adjustmentNote:
        typeof requestedWeeks === "number" && chosenWeeks !== requestedWeeks
...
export function applyTimelineRecommendation(input: RunnerInput, recommendation: TimelineRecommendation): RunnerInput {
  return {
    ...input,
