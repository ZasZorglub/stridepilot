import type { Phase, PhaseBlock, PhasePlan, PhasePurpose, PhaseWeek, PlanType, RunnerClassification, RunnerInput, TimelinePhase } from "./models";
import { deriveCalendarWeekCount } from "../calendar-week";

type PhaseAllocations = Record<Phase, number>;

export function getPhaseForWeek(week: number, totalWeeks: number): TimelinePhase {
  const boundedTotal = Math.max(6, totalWeeks);
  const boundedWeek = Math.max(1, Math.min(week, boundedTotal));
  if (boundedWeek === boundedTotal) return "race";
  ...
  if (boundedWeek <= baseEnd) return "base";
  if (boundedWeek <= buildEnd) return "build";
  if (boundedWeek <= specificEnd) return "specific";
  if (boundedWeek <= peakEnd) return "peak";
  return "taper";
}
...
): Pick<PhaseWeek, "targetRuns" | "targetQualitySessions" | "notes"> {
