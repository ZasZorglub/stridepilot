...
function goalLabel(plan: EnginePlan): string {
  ...
  if (plan.input.goalType === "finish_without_walking") return `${distance} uden stop`;
  if (plan.input.goalType === "improve_time") return `${distance} med fokus på at forbedre tiden`;
  if (plan.input.goalType === "target_time") return `${distance} mod en bestemt måltid`;
  if (plan.input.goalType === "return_to_running") return `at komme godt tilbage til løb`;
  return `${distance} som gennemførselsmål`;
}

function phaseFocusLabel(phase: Phase): string {
  if (phase === "base") return "bygge rytme og base";
  if (phase === "build") return "bygge volumen og robusthed";
  if (phase === "specific") return "gøre træningen mere målspecifik";
  if (phase === "peak") return "ramme de skarpeste træningsuger uden at tippe over";
  return "skabe friskhed og holde rytmen skarp";
}

function formatGoalTime(goalTime?: string): string | undefined {
  if (!goalTime) return undefined;
  return goalTime.includes(":") ? goalTime : `${goalTime}`;
}

export function explainTrainingPlan(plan: EnginePlan, runnerProfile: RunnerClassification = plan.classification): string {
...
export function explainWeek(week: BuiltWeek, phase: Phase = week.phase): string {
...
export function explainWorkout(workout: BuiltSession): string {
...
export function explainAdaptation(adaptationReason: string | AdaptationResult): string {
...
export function raceStrategy(raceDistance: RaceDistance, goalTime?: string): string {
...
export function injuryPreventionAdvice(): string {
...
export function motivationMessage(
  ...
  progress: CoachingProgressSnapshot,
): string {
...
export function buildCoachExplanation(plan: EnginePlan): CoachExplanation {
