...
function normalizePain(pain: FeedbackSignal["pain"]): number {
  return pain <= 3 ? pain : Math.min(3, Math.round(pain / 1.5));
}

function effortToRpe(entry: FeedbackSignal): number {
  if (typeof entry.rpe === "number") return entry.rpe;
  if (typeof entry.effort === "number") return 1 + entry.effort * 1.5;
  if (entry.perceivedDifficulty === "too_hard") return 9;
  if (entry.perceivedDifficulty === "hard") return 7.5;
  if (entry.perceivedDifficulty === "too_easy") return 3.5;
  return 5.5;
}

function scaleCurve(curve: number[], startWeekIndex: number, factor: number, step = 0.5): number[] {
  return curve.map((value, index) => {
    if (index + 1 < startWeekIndex) return value;
    const scaled = value * factor;
    return step > 0 ? Math.round(scaled / step) * step : scaled;
  });
}

function scaleIntensityCurve(curve: number[], startWeekIndex: number, factor: number): number[] {
...
function likelyBreakInTraining(recentWorkouts: FeedbackSignal[]): boolean {
...
function completionRateFromRecent(recent: FeedbackSignal[]): number {
  return average(recent.map((entry) => entry.completionPct / 100));
}

function fatigueScoreFromRecent(recent: FeedbackSignal[]): number {
  return average(
    recent.map((entry) => {
...
function painScoreFromRecent(recent: FeedbackSignal[]): number {
  return average(recent.map((entry) => normalizePain(entry.pain)));
}

function statusToMode(status: RunnerStatus): AdaptationDecision["mode"] {
  if (status === "progressing_well" || status === "undertraining") return "progress";
  if (status === "struggling") return "repeat_week";
  if (status === "overreaching" || status === "injury_risk") return "recovery_week";
  if (status === "inconsistent") return "repeat_week";
  return "hold";
}
...
export function evaluateRunnerStatus(
  ...
  plan: EnginePlan,
  runnerProfile: RunnerClassification,
): RunnerStatusEvaluation {
...
export function adaptPlan(plan: EnginePlan, status: RunnerStatus): AdaptationResult {
  ...
  if (status === "progressing_well") {
    ...
    updatedCurves.sessionsPerWeekCurve = updatedCurves.sessionsPerWeekCurve.map((value, index) =>
      index + 1 >= startWeekIndex ? Math.min(value + 1, 5) : value,
    );
...
export function adaptTrainingPlan(plan: EnginePlan, status: RunnerStatus): AdaptationResult {
  return adaptPlan(plan, status);
}

export function updatePlanAfterFeedback(plan: EnginePlan, recentWorkouts: FeedbackSignal[]): AdaptationResult & { status: RunnerStatus } {
