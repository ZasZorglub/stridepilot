...
function weekValidationSeverity(plan: EnginePlan, weekIndex: number): "none" | "warning" | "hard_fail" {
  const results = plan.vNextValidation?.results.filter((result) => result.weekIndex === weekIndex) ?? [];
  if (results.some((result) => result.severity === "hard_fail")) return "hard_fail";
  if (results.some((result) => result.severity === "warning" || result.severity === "auto_adjust")) return "warning";
  return "none";
}

function isProtectedRunner(plan: EnginePlan): boolean {
  return Boolean(
    plan.returnToRunningState?.active ||
...
function progressionGateForWeek(plan: EnginePlan, weekIndex: number): ProgressionGate {
  ...
  if (!week) return "restricted";
  if (week.isRaceWeek || week.phase === "taper") return "restricted";
  if (validation === "hard_fail") return "restricted";
  if (week.isCutback) return "hold";
  if (returnWeekState && !returnWeekState.continuityGatePassed) return "restricted";
  if (validation === "warning") return "hold";
  if (week.phase === "peak") return "hold";
  return "open";
}

function buildBoundaryKinds(plan: EnginePlan, weekIndex: number): WeekAdaptationHooks["boundaryKinds"] {
  const week = plan.weeks[weekIndex - 1];
  const previous = plan.weeks[weekIndex - 2];
  const kinds: WeekAdaptationHooks["boundaryKinds"] = [];
  if (!week) return kinds;
  if (!previous || previous.phase !== week.phase) kinds.push("phase_start");
  if (week.phase === "build" && previous?.phase === "base") kinds.push("pre_specific");
  if (previous?.isCutback) kinds.push("post_cutback");
  if (previous?.isRaceWeek) kinds.push("post_race");
  if (plan.returnToRunningState?.active) {
    const returnWeekState = plan.returnToRunningState.weeklyStates.find((entry) => entry.weekIndex === weekIndex);
    if (returnWeekState && !returnWeekState.continuityGatePassed) kinds.push("protected_hold");
  }
  const next = plan.weeks[weekIndex];
  if (!next || next.phase !== week.phase) kinds.push("phase_end");
  return kinds;
}

function buildWeekHooks(plan: EnginePlan, weekIndex: number): WeekAdaptationHooks {
  const week = plan.weeks[weekIndex - 1];
  ...
  const repeatWeekCandidate =
    week.isCutback ||
    gate === "hold" ||
    validation === "warning" ||
    (protectedRunner && !continuityGatePassed);

  const recoveryMicrocycleCandidate =
    week.isCutback ||
    week.phase === "taper" ||
    validation !== "none" ||
...
function buildPlanBoundaries(plan: EnginePlan): AdaptationBoundary[] {
  const boundaries: AdaptationBoundary[] = [];
...
export function buildAdaptationHooks(plan: EnginePlan): PlanAdaptationHooks {
  for (const week of plan.weeks) {
