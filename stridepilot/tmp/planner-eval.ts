...
import type { Goal, RunnerProfile as AppRunnerProfile } from "../src/lib/types";
...
function totalRunMinutes(session: { steps: Array<{ type: string; durationSec: number }> }) {
  return Math.round(session.steps.filter((step) => step.type === "run").reduce((sum, step) => sum + step.durationSec, 0) / 60);
}

function sessionType(session: { title: string; notes?: string }) {
  ...
  if (text.includes("langt") || text.includes("udholdenhed")) return "long";
...
function summarizeAppPlan(appPlan: ReturnType<typeof mapCoachPlanToAppPlan>) {
  ...
  return {
    ...
    workoutTypeCounts: appPlan.sessions.reduce<Record<string, number>>((acc, session) => {
...
for (const persona of personas) {
  ...
  const plan = buildGoalPlan(coachProfile, deriveCoachGoal(persona.goal, persona.runnerProfile));
  const appPlan = mapCoachPlanToAppPlan(plan, plan.goal);
